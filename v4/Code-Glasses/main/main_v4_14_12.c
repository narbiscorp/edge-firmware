/*******************************************************************************
 * Smart Glasses Firmware v4.14.12
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.12"
static const char *TAG = "SG_v4.14.12";
...
static void led_task(void *param) {
    ...
    while (1) {
         ...
         *
         * Linear inverse mapping. No floor for now — if it feels too
         * binary (lens slamming open and closed when coh crosses a
         * threshold), we can add a floor constant or smoothing.
         *
         ...
         *******************************************************************/
            ...
            uint32_t duty = (uint32_t)brightness * (100 - (uint32_t)coh) / 100;
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.12 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static uint8_t  ppg_batch_count = 0;      /* Samples accumulated in current batch */
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.12 started @ %d Hz (GPIO%d)",
