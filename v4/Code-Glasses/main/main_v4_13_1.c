 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.13.1"
static const char *TAG = "SG_v4.13.1";
...
static void coh_compute(void) {
    ...
    if (total > 1e-6f) {
        ...
        coherence = ratio * ratio * 250.0f * (float)COH_GRID_N;  /* scale factor tuned in simulation */
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.13.1 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.13.1 started @ %d Hz (GPIO%d)",
