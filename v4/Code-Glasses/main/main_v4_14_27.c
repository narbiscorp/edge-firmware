/*******************************************************************************
 * Smart Glasses Firmware v4.14.27
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.27"
static const char *TAG = "SG_v4.14.27";
...
/* v4.14.27: coherence difficulty forward-declared at file-top so
 * led_task and process_command (which both live above the coherence
 * module in this file) can reference these. Same pattern as
 * g_sensor_gate_ok in v4.14.4 and ppg_batch_count in v4.14.13.
 *
 * Difficulty (HeartMath-style): raw coherence score stays identical
 * across all levels. This only affects:
 *   (1) the COHERENCE_LENS opacity mapping curve (led_task)
 *   (2) the display-zone thresholds on the dashboard (client-side)
 *
 * Lens knee points — lens stays fully dark until coherence reaches
 * coh_start, then linearly clears through to coh_end.
 *   Easy   (0): start  0, end 100  — classic linear (v4.14.24 behavior)
 *   Medium (1): start 30, end  70
 *   Hard   (2): start 50, end  85
 *   Expert (3): start 65, end  95
 *
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.27 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.27 started @ %d Hz (GPIO%d)",
