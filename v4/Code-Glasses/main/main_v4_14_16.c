/*******************************************************************************
 * Smart Glasses Firmware v4.14.16
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.16"
static const char *TAG = "SG_v4.14.16";
...
#define PPG_RAW_AC_MIN          200     /* threshold to ENTER OK state */
#define PPG_RAW_AC_MIN_STICKY   80      /* lower threshold to STAY in OK state */
...
static bool ppg_sensor_looks_present(void) {
    /* v4.14.14: warm-up behavior — return FALSE, not true, while we
     * don't yet have enough ADC stats to judge. "No sensor until
     * proven otherwise" matches the conservative default on
     * g_sensor_gate_ok and prevents spurious PPG-auto entry at boot. */
    ...
    /* v4.14.16: hysteresis. Previously a single 500ms stats window that
     * fell between heartbeats (especially on slow HR where one beat
     * cycle is >500ms) could see low AC p-p and flap the gate out of
     * OK. That flap reset the detector and cleared the coherence ring
     * every couple seconds, making firmware HRV and coh unstable.
     *
     * Now: entering OK still requires the strict PPG_RAW_AC_MIN (200)
     * so spurious low-amplitude noise doesn't trigger a false connect.
     * But STAYING OK only requires the looser PPG_RAW_AC_MIN_STICKY (80)
     * so normal between-beat lulls don't kick us out.
     *
     * A real unplug will produce DC-out-of-range almost immediately
     * (signal rails to 0 or ~3500-ish depending on floating state),
     * so DC gate remains the primary disconnect detector. */
    uint16_t ac_threshold = g_sensor_gate_ok
        ? PPG_RAW_AC_MIN_STICKY
        : PPG_RAW_AC_MIN;
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.16 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.16 started @ %d Hz (GPIO%d)",
