 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.13.2"
static const char *TAG = "SG_v4.13.2";
...
static void coh_compute(void) {
    ...
    /* LF resonance peak (HeartMath-style coherence):
     * Find argmax within LF band, compute coherence as (peak² / total²) × 250.
     * Clamped 0-100. This mirrors what the dashboard currently calculates
     * and what HeartMath's published papers define.
     *
     * Why squared ratio: a perfectly concentrated signal (all power in one bin)
     * scores ~250, so × 250/n scaling lands reasonable rhythms near 40-80. */
    int peak_bin = 3;
    ...
    if (total > 1e-6f) {
        ...
        /* v4.13.2 FIX: previous v4.13.0-v4.13.1 had an extra × N factor
         * that made every coherence value 256× too large, saturating at
         * 100. Python simulation (validated against client-side math)
         * uses just × 250. Firmware now matches. */
        coherence = ratio * ratio * 250.0f;
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.13.2 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.13.2 started @ %d Hz (GPIO%d)",
    ...
    while (1) {
        ...
        if (now_ms - last_hb_ms >= 5000) {
            last_hb_ms = now_ms;
            ble_log("ppg t=%us raw=%u gate=%s MAb=%.1f beats=%u jitmax=%uus over=%u",
