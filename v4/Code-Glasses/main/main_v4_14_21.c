/*******************************************************************************
 * Smart Glasses Firmware v4.14.21
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.21"
static const char *TAG = "SG_v4.14.21";
...
/* v4.14.20: simplified gate — sustained raw ADC span.
 *
 * Previous versions tried increasingly sophisticated approaches
 * (hysteresis in v4.14.16, rhythmicity in v4.14.18, amplitude floor
 * in v4.14.19) and kept getting fooled by noise on boot. Stripped
 * back to the simplest thing that could possibly work: require the
 * 500ms raw-ADC span to be SUSTAINED above a threshold for several
 * consecutive evaluations.
 *
 * With no sensor: observed span stays around 100-200 counts (noise).
 * With a real sensor on skin: span is >400 counts easily and stays
 * there as long as the finger is in contact.
 *
 * Noise can transiently spike the span above 400 (a single big
 * coupling event) but it won't SUSTAIN. Requiring sustained gives us
 * robustness without any filtering, rhythmicity, or detector-side
 * machinery.
 *
 * Parameters:
 *   PPG_RAW_DC_MIN/MAX: unchanged, hard disconnect rail check.
 *   PPG_GATE_SPAN_MIN:  raw ADC peak-to-peak threshold.
 *   PPG_GATE_OK_N:      how many consecutive OK evaluations required
 *                       to promote the gate to OK.
 *   PPG_GATE_BAD_N:     how many consecutive BAD evaluations required
 *                       to demote the gate to not-OK.
 *
 * ppg_sensor_looks_present runs roughly every PPG sample (50Hz), so
 * GATE_OK_N=250 → 5 seconds sustained. GATE_BAD_N=250 → 5 seconds
 * sustained. v4.14.21: bumped both from 20/30 to 250/250 — user
 * requested 5-second confirmation time for both plug-in and unplug
 * so momentary noise or finger lifts don't cause gate flaps.
 */
#define PPG_RAW_DC_MIN          300
#define PPG_RAW_DC_MAX          3900
#define PPG_GATE_SPAN_MIN       400
#define PPG_GATE_OK_N           250
#define PPG_GATE_BAD_N          250
...
/* v4.14.20: sensor-presence gate, simplified.
 *
 * Single metric: raw ADC span (peak-to-peak) over the 500ms stats
 * window. Must be SUSTAINED above PPG_GATE_SPAN_MIN for PPG_GATE_OK_N
 * consecutive evaluations before the gate is promoted to OK. To
 * demote, must be below for PPG_GATE_BAD_N consecutive evaluations.
 *
 * Sustained-counter approach discards noise spikes automatically
 * because a single-sample excursion doesn't accumulate.
 *
 * DC rail check remains as a hard disconnect signal — if the line is
 * floating or shorted, mean will be outside [300, 3900] and we
 * short-circuit to not-OK regardless of sustained counter state.
 *
 * Run frequency: called from ppg_detect on every 50Hz sample, so
 * OK_N=20 → ~400ms sustained-above required, BAD_N=30 → ~600ms
 * sustained-below required. */
static bool ppg_sensor_looks_present(void) {
    static uint16_t good_streak = 0;
    static uint16_t bad_streak  = 0;

    /* v4.14.14: warm-up — return false until we have ADC stats. */
    if (adc_stats_count < 5) return false;

    /* Compute min/max/mean over the ring. */
    uint16_t mn = 4095, mx = 0;
    uint32_t sum = 0;
    for (int i = 0; i < adc_stats_count; i++) {
        uint16_t v = adc_stats_ring[i];
    ...
    /* Hard disconnect: if DC rails, reset both streaks and return
     * immediately not-OK. */
    if (mean < PPG_RAW_DC_MIN || mean > PPG_RAW_DC_MAX) {
        good_streak = 0;
        bad_streak  = PPG_GATE_BAD_N;  /* latch demotion */
        return false;
    }

    /* Update sustained counters based on span. */
    if (span >= PPG_GATE_SPAN_MIN) {
        if (good_streak < PPG_GATE_OK_N) good_streak++;
        bad_streak = 0;
    } else {
        if (bad_streak < PPG_GATE_BAD_N) bad_streak++;
        good_streak = 0;
    }

    /* Return current gate state. Promote on sustained-good, demote on
     * sustained-bad; otherwise hold previous state. */
    if (good_streak >= PPG_GATE_OK_N) return true;
    if (bad_streak  >= PPG_GATE_BAD_N) return false;
    return g_sensor_gate_ok;   /* hold previous */
}
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.21 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.21 started @ %d Hz (GPIO%d)",
