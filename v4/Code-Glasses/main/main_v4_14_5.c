/*******************************************************************************
 * Smart Glasses Firmware v4.14.5
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.5"
static const char *TAG = "SG_v4.14.5";
 ...
 ******************************************************************************/
typedef enum {
    PPG_PROG_HEARTBEAT                 = 0,  /* LED_MODE_PULSE_ON_BEAT */
    PPG_PROG_COHERENCE_BREATHE         = 1,  /* LED_MODE_COHERENCE_BREATHE */
    PPG_PROG_COHERENCE_BREATHE_STROBE  = 2,  /* LED_MODE_COHERENCE_BREATHE_STROBE */
    PPG_PROG_COUNT                     = 3,
} ppg_program_t;
 ...
 ******************************************************************************/
#define INDICATOR_PULSE_MS      1000   /* Total time for one fade-in + fade-out */
...
static volatile indicator_state_t indicator = { 0, 0, 0 };

/* v4.14.5: fast-path check used by the strobe ISR to know when to stop
 * writing effective_duty. The ISR runs every 100µs and writes
 * effective_duty from its own strobe DDS calculation, which would
 * clobber the indicator's envelope in programs 2/3 (BREATHE_STROBE,
 * STROBE). When indicator is active, ISR skips the effective_duty
 * write and lets led_task's indicator_tick output dominate.
 *
 * Single-byte volatile read is atomic on ESP32 — safe from ISR. */
static inline bool indicator_is_active(void) {
    return indicator.pulses_remaining > 0 || indicator.hold_until_tick != 0;
}
 ...
 * hold_ms = time to hold CLEAR (0% duty) after the last pulse before
 * returning control to the normal program. Used for the PPG-auto
 * handshake (5 pulses then 3s clear then PPG program 1).
 *
...
static void indicator_trigger(uint8_t count, uint32_t hold_ms) {
    ...
    indicator.pulse_start_tick = xTaskGetTickCount();
    indicator.hold_until_tick = 0;  /* hold computed after last pulse */
    /* Stash hold duration in hold_until_tick temporarily — will be
     * converted to absolute tick after last pulse ends. Non-zero means
     * "a hold is requested." */
    if (hold_ms > 0) {
        /* Encode the requested hold_ms by storing the absolute tick of
         * the LAST pulse end + hold. Since pulses are INDICATOR_PULSE_MS
         * each and count pulses total, the last pulse ends at:
         *   pulse_start_tick + count × INDICATOR_PULSE_MS
         * After which hold_ms of clear should elapse before release. */
        indicator.hold_until_tick =
            indicator.pulse_start_tick
            + pdMS_TO_TICKS((uint32_t)count * INDICATOR_PULSE_MS + hold_ms);
    }
}

/* Compute current indicator duty output (0-255 fraction, 0 = clear, 255 = full).
 * Returns -1 if indicator is not active (caller should use normal program output).
...
static int indicator_tick(void) {
    if (indicator.pulses_remaining == 0) {
        ...
        if (indicator.hold_until_tick != 0 &&
            xTaskGetTickCount() < indicator.hold_until_tick) {
            return 0;
        }
    ...
    if (elapsed_ms >= INDICATOR_PULSE_MS) {
        ...
        if (indicator.pulses_remaining > 0) {
            ...
            /* All pulses done. Hold logic is handled above on the next tick
             * when pulses_remaining == 0 and hold_until_tick is set. */
            return 0;
        }
    }
    ...
    /* v4.14.5 FIX: effective_duty is a 0-100 PERCENT value (see duty_to_raw),
     * not a 0-255 byte. Previous versions multiplied by 2.55 which overshot
     * the clamp at 100, making the indicator maxed-out for the whole pulse
     * instead of ramping.
     *
     * Polarity: duty=0 is clear (lens passes light), duty=100 is full tint
     * (lens dark). Indicator ramps 0 → peak → 0, so visually: clear → dark
     * → clear. This matches the intended "pulse dark on clear background"
     * cue. */
...
static void ppg_apply_program(ppg_program_t p) {
    ...
    switch (p) {
        ...
        case PPG_PROG_COHERENCE_BREATHE_STROBE:
            ...
            if (session_active) strobe_start();
            ESP_LOGI(TAG, "PPG Program 3: COHERENCE BREATHE+STROBE (coh-modulated)");
            ble_log("ppg prog 3: coh breathe+strobe");
            break;
...
static void ppg_auto_check(void) {
    ...
    if (!ppg_auto_active) {
        ...
        if (ppg_present_streak_s >= PPG_PRESENCE_DEBOUNCE_S) {
             ...
             *
             *   1. Session duration — give the user a full 10 minutes of
             *      biofeedback starting now, not counted from boot. If
             *      they plugged the sensor in 8 minutes after power-on,
             *      previously the session would only have 2 minutes left.
             *
             ...
             *      a short hall tap.
             *
             * This matches the intended UX: "sensor in, 1 minute to connect,
             * otherwise radio off; 10 minutes of session; tap to wake radio." */
 ...
 * recomputing.
 *
 * v4.14.4: g_sensor_gate_ok definition moved to file-top state block
 * so ppg_auto_check (which runs in coherence_task, defined earlier in
 * the file) can read it without a forward declaration dance. */
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.5 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.5 started @ %d Hz (GPIO%d)",
