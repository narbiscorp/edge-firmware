/*******************************************************************************
 * Smart Glasses Firmware v4.9.10
 * 
 ...
 * 
 * FEATURES:
 * - AC differential drive: GPIO26/GPIO27 via gptimer 100µs ISR (500Hz AC)
 ...
 * 
 * CHANGELOG v4.9.10:
 * - AC frequency set to 500Hz (AC_HALF_TICKS=10) — compromise between lens
 *   charging time (RC of driver+lens) and beat-frequency rejection at high
 *   strobe rates. 1kHz (v4.9.9) was too fast: lens couldn't fully charge,
 *   reducing effective tint. 100Hz (v4.9.8) was too slow: beat artifacts
 *   above ~20Hz strobe.
 * - Cached strobe dark threshold retained from v4.9.9
 ******************************************************************************/
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.9.10"
static const char *TAG = "SG_v4.9.10";
...
#define AC_HALF_TICKS           10      /* 10 × 100µs = 1ms = 500Hz AC */
 ...
 * 
 * AC: toggle every AC_HALF_TICKS (10 × 100µs = 1ms = 500Hz)
...
static bool IRAM_ATTR drive_timer_cb(gptimer_handle_t timer,
    ...
    if (led_mode == LED_MODE_STROBE && session_active && !in_ota_mode) {
        ...
        effective_duty = (strobe_acc < strobe_dark_thresh) ? brightness : 0;
    }
...
static void drive_timer_init(void) {
    ...
    ESP_LOGI(TAG, "Drive timer started (100µs gptimer ISR / 500Hz AC)");
}
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  AC Drive: 500Hz gptimer ISR (GPIO26/27)");
