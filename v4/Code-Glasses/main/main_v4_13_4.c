/*******************************************************************************
 * Smart Glasses Firmware v4.13.4
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.13.4"
static const char *TAG = "SG_v4.13.4";
 ...
 *
 * AC_MIN ensures some pulse envelope exists — a flat DC (sensor
 * plugged in but floating or no-skin) produces <5 code p-p which
 * this threshold rejects. 10 leaves margin for weak-contact cases.
 *
...
static void coh_compute(void) {
     ...
     * Using inclusive-inclusive ranges as published. */
    ...
    /* Normalize band powers to fit u16 — our magnitudes can be very large
     * depending on signal amplitude. Clamp to u16 max. */
    float scale = 1.0f;
    float maxband = total;
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.13.4 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.13.4 started @ %d Hz (GPIO%d)",
