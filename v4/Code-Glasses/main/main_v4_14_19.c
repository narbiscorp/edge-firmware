/*******************************************************************************
 * Smart Glasses Firmware v4.14.19
 *
 ...
 * 
 * CHANGELOG v4.14.19:
 * - Added amplitude floor to the sensor gate. v4.14.18's rhythmicity
 *   check caught most false positives but was still being fooled at
 *   boot in noisy environments: user reported "still going into
 *   training mode at startup with no sensor" with observed raw ADC
 *   span=128, mean=718.
 *
 *   Why rhythm alone is insufficient: bandpass-filtered noise from
 *   mains/USB coupling can produce quasi-periodic ghost beats. If the
 *   noise happens to modulate at a consistent rate (mains harmonics,
 *   USB bus frequencies aliasing against 50Hz sampling), 4 of 8 beats
 *   can land in the heart-range IBI window with low CV. Rhythmicity
 *   says "yes" but amplitude is tiny.
 *
 *   Fix: add a bandpass amplitude floor (PPG_AMP_MIN = 25.0) that
 *   ma_beat must exceed at gate evaluation time. Real PPG drives
 *   ma_beat into the hundreds; noise-triggered ghost beats rarely
 *   exceed 10-15. Three-stage gate now:
 *     1. DC rail check: mean ∈ [300, 3900]
 *     2. Amplitude floor: ma_beat ≥ PPG_AMP_MIN
 *     3. Rhythmicity: ≥4 of 8 IBIs heart-like with CV ≤ 40%
 *
 *   All three must pass for gate to be OK. Any one failing keeps
 *   gate closed / flips it closed.
 *
 * CHANGELOG v4.14.18:
 * - Sensor-presence gate rewritten. Previous p-p-based gate was
 *   triggering false "sensor connected" events in electrically
 *   noisy environments (USB hash, floating GPIO pickup, mains
 *   coupling). A single noise spike in the 500ms gate window was
 *   enough to produce p-p > 200 and trip the gate.
 *
 *   New architecture:
 *     Stage 1: DC rail check. If ADC mean is outside [300, 3900],
 *              sensor is definitely disconnected (line floating or
 *              shorted). Short-circuits detection and clears state.
 *     Stage 2: Rhythmicity check. The beat detector now ALWAYS runs
 *              when DC is in range. Every IBI it produces is pushed
 *              to a small rhythm ring. Sensor is only considered
 *              "present" when we see ≥4 of last 8 IBIs in the
 *              heart-rate range (40-180 BPM) with CV ≤ 40%.
 *
 *   The rhythmicity check is the real discriminator: broadband noise
 *   can produce occasional ghost beats, but it cannot produce 4
 *   heart-range IBIs with low variance over several seconds. Real
 *   signal does that trivially.
 *
 *   Tradeoff: plug-in detection is slower (~4-6 seconds vs ~2 seconds
 *   before) because we need several real beats to confirm. Unplug
 *   detection via DC rail is still instant. False-positive rate in
 *   noisy environments should be effectively zero.
 *
 *   Removed: PPG_RAW_AC_MIN, PPG_RAW_AC_MIN_STICKY. No longer used.
 *   Replaced by RHYTHM_N_IBIS, RHYTHM_MIN_VALID, RHYTHM_IBI_MIN/MAX,
 *   RHYTHM_STALE_MS, RHYTHM_CV_PCT_MAX.
 *
 *   Log messages updated: "sensor present (rhythm confirmed)" on
 *   entry, "sensor lost (no rhythm)" on exit via lost rhythm, and
 *   "sensor disconnected (DC rail)" on rail-level unplug.
 *
 * CHANGELOG v4.14.17:
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.19"
static const char *TAG = "SG_v4.14.19";
...
#define PPG_PRESENCE_DEBOUNCE_S 1      /* v4.14.18: reduced 2 → 1. Rhythm gate
                                        * itself already needs 4+ beats to flip
                                        * true, so additional multi-second
                                        * debounce here is redundant. */
#define PPG_ABSENCE_DEBOUNCE_S  3      /* Sensor-gate must be stably NOT-OK
                                        * for this many seconds before we
                                        * switch OUT of ppg-auto mode.
                                        * Longer than the "present" debounce
                                        * because a momentary finger lift
                                        * shouldn't drop us back to a totally
                                        * different program. */
...
#define INDICATOR_POST_DELAY_MS 1000   /* v4.14.8: clear hold AFTER the last
                                        ...
                                        * POST_DELAY. */
#define BOOT_INDICATOR_DELAY_MS 3000   /* v4.14.3: delay boot indicator so that
...
static void ppg_auto_check(void) {
    ...
    if (!ppg_auto_active) {
        ...
        /* In PPG mode — look for sustained unplug. */
        if (ppg_absent_streak_s >= PPG_ABSENCE_DEBOUNCE_S) {
            ppg_auto_active = false;
            ESP_LOGI(TAG, "PPG sensor unplugged — restoring hall program %d",
                     (int)saved_hall_program);
            ble_log("ppg auto: OFF (restore prog %d)", (int)saved_hall_program);
            apply_program(saved_hall_program);
            /* v4.14.2: show which hall program we restored to */
            indicator_trigger((uint8_t)(saved_hall_program + 1), 0);
        }
    }
}
...
static void led_task(void *param) {
     ...
     * v4.14.3: deferred by BOOT_INDICATOR_DELAY_MS (3s) so that if the
     * sensor is already connected at boot, PPG-auto (which takes 2s to
     * detect the gate being OK) has a chance to activate first. When
     * PPG-auto activates, it sets boot_indicator_shown = true to
     * suppress the deferred boot indicator. If the user boots without
     * a sensor, the boot indicator fires normally 3 seconds in. */
    ...
    while (1) {
         ...
         *
         * Breathing parameters are HARDCODED LOCALLY (not from the global
         * breathe_* vars) so BLE tweaks to those vars don't change the
         * resonant pacer shape:
         *     6 BPM   (10-second cycle)
         *     40% inhale / 60% exhale
         *     sine waveform
         *     no holds
         *
         ...
         * Reads coh_state.coherence atomically (single-byte read is safe
         * on 32-bit ESP32 — coherence_task writes it as a u8 store).
         *******************************************************************/
        else if (led_mode == LED_MODE_COHERENCE_BREATHE ||
                 led_mode == LED_MODE_COHERENCE_BREATHE_STROBE) {
            const uint32_t CB_CYCLE_MS   = 10000;   /* 6 BPM = 10s cycle */
            const uint32_t CB_INHALE_MS  = 4000;    /* 40% inhale */
            const uint32_t CB_EXHALE_MS  = 6000;    /* 60% exhale */

            uint32_t t = (tick_count * LED_TICK_MS) % CB_CYCLE_MS;
            float frac = 0.0f;

            if (t < CB_INHALE_MS) {
                float p = (float)t / (float)CB_INHALE_MS;
                frac = (1.0f - cosf((float)M_PI * p)) / 2.0f;
            } else {
                float p = (float)(t - CB_INHALE_MS) / (float)CB_EXHALE_MS;
                frac = (1.0f + cosf((float)M_PI * p)) / 2.0f;
            }
         ...
         *******************************************************************/
            ...
            uint32_t duty = (uint32_t)brightness * (uint32_t)(100.0f - s) / 100;
            if (duty > 100) duty = 100;
...
/* Gate thresholds. v4.14.16: hysteresis added.
 *
 * v4.14.18: the raw AC p-p check is no longer the primary indicator.
 * Broadband noise (USB hash, mains pickup on a floating ADC line)
 * easily clears any p-p threshold, because a single 1000-count spike
 * in a 500ms window makes p-p = 1000. That was producing false
 * "sensor connected" events at boot in noisy environments.
 *
 * Instead we now use a rhythmicity check — see ppg_rhythm_* state and
 * ppg_rhythm_grade() below. Real PPG produces beats 500-1500ms apart
 * with consistent IBIs. Noise produces no beats, or chaotic IBIs. The
 * rhythmicity grade is what promotes the gate to OK.
 *
 * The raw DC range check is retained as a hard "definite disconnect"
 * signal: if the ADC reads rail-to-0 or rail-to-max (signal line
 * floating or shorted), the sensor is physically disconnected
 * regardless of what the rhythmicity tracker says. */
#define PPG_RAW_DC_MIN          300
#define PPG_RAW_DC_MAX          3900

/*******************************************************************************
 * SENSOR RHYTHMICITY GATE (v4.14.18)
 *
 * Tracks recent beat intervals and grades how "heart-like" they are.
 * A real sensor plugged in to a real finger produces beats roughly
 * every 500-1500 ms with low IBI variance. Anything else — no beats,
 * beats at impossible rates, chaotic IBIs — fails the grade.
 *
 * Pushed to by ppg_detect each time a beat is emitted. Graded by
 * ppg_rhythm_grade() which is cheap (small ring, basic stats) and
 * called from ppg_sensor_looks_present() once per gate evaluation.
 *
 * Parameters:
 *   RHYTHM_N_IBIS: how many recent IBIs to look at (8 = last ~5-10s
 *                  of data at normal HR).
 *   RHYTHM_MIN_VALID: how many of those must look heart-like for the
 *                    sensor to be considered present.
 *   RHYTHM_IBI_MIN / MAX: valid IBI window in ms (corresponds to
 *                        ~40-180 BPM).
 *   RHYTHM_STALE_MS: a beat older than this is "stale" — counts as
 *                    invalid even if its IBI was valid.
 *   RHYTHM_CV_PCT_MAX: coefficient-of-variation limit on the valid
 *                     IBIs, in percent. Real HRV is typically 5-15%
 *                     CV; noise-triggered beats go much higher.
 ******************************************************************************/
#define RHYTHM_N_IBIS        8
#define RHYTHM_MIN_VALID     4       /* need 4 of last 8 valid to call it "present" */
#define RHYTHM_IBI_MIN       333     /* 180 BPM upper */
#define RHYTHM_IBI_MAX       1500    /* 40 BPM lower */
#define RHYTHM_STALE_MS      8000    /* beat older than 8s doesn't count */
#define RHYTHM_CV_PCT_MAX    40      /* CV > 40% → chaotic, probably noise */

/* v4.14.19: amplitude floor for the rhythm gate. Bandpass ma_beat
 * must exceed this for the gate to promote to OK. Real PulseSensor
 * signals drive ma_beat into the hundreds. Noise-triggered ghost
 * beats from pickup on a disconnected pin can have ma_beat somewhere
 * around PPG_MIN_MA_BEAT (4.0) up to maybe 10-15. PPG_AMP_MIN sits
 * above the plausible noise range but well below real signal. If
 * real signals don't reach this on some hardware revision, lower it. */
#define PPG_AMP_MIN          25.0f
...
static rhythm_beat_t ppg_rhythm_ring[RHYTHM_N_IBIS] = {0};
static uint8_t       ppg_rhythm_idx = 0;
static uint8_t       ppg_rhythm_count = 0;   /* up to RHYTHM_N_IBIS */

/* v4.14.19: most recent ma_beat value from the detector. Exported
 * so the gate can check bandpass amplitude at evaluation time. */
static volatile float ppg_last_ma_beat = 0.0f;

/* Push an IBI onto the rhythmicity ring. Called from ppg_detect on
 * every confirmed beat. */
static void ppg_rhythm_push(uint16_t ibi_ms, uint32_t ts_ms) {
    ppg_rhythm_ring[ppg_rhythm_idx].ibi_ms = ibi_ms;
    ppg_rhythm_ring[ppg_rhythm_idx].ts_ms  = ts_ms;
    ppg_rhythm_idx = (ppg_rhythm_idx + 1) % RHYTHM_N_IBIS;
    if (ppg_rhythm_count < RHYTHM_N_IBIS) ppg_rhythm_count++;
}

/* Clear the rhythmicity ring. Called on gate OK→not-OK transition
 * so a subsequent reconnect starts fresh. */
static void ppg_rhythm_clear(void) {
    ppg_rhythm_idx = 0;
    ppg_rhythm_count = 0;
    for (int i = 0; i < RHYTHM_N_IBIS; i++) {
        ppg_rhythm_ring[i].ibi_ms = 0;
        ppg_rhythm_ring[i].ts_ms  = 0;
    }
}

/* Grade the current state of the rhythmicity ring. Returns true if
 * we see enough recent, valid, low-variance beats to confirm sensor
 * presence. */
static bool ppg_rhythm_looks_real(uint32_t now_ms) {
    if (ppg_rhythm_count < RHYTHM_MIN_VALID) return false;

    uint8_t valid_count = 0;
    uint32_t ibi_sum = 0;
    for (int i = 0; i < ppg_rhythm_count; i++) {
        uint16_t ibi = ppg_rhythm_ring[i].ibi_ms;
        uint32_t age = now_ms - ppg_rhythm_ring[i].ts_ms;
        if (ibi >= RHYTHM_IBI_MIN && ibi <= RHYTHM_IBI_MAX
            && age <= RHYTHM_STALE_MS) {
            valid_count++;
            ibi_sum += ibi;
        }
    }
    if (valid_count < RHYTHM_MIN_VALID) return false;

    /* Coefficient of variation: σ / μ × 100. Computed only over the
     * in-range, non-stale beats. Chaotic noise-triggered detections
     * have CV > 50% easily; real HRV is typically 5-15%. */
    uint32_t mean = ibi_sum / valid_count;
    uint64_t sq_dev = 0;
    for (int i = 0; i < ppg_rhythm_count; i++) {
        uint16_t ibi = ppg_rhythm_ring[i].ibi_ms;
        uint32_t age = now_ms - ppg_rhythm_ring[i].ts_ms;
        if (ibi >= RHYTHM_IBI_MIN && ibi <= RHYTHM_IBI_MAX
            && age <= RHYTHM_STALE_MS) {
            int32_t dev = (int32_t)ibi - (int32_t)mean;
            sq_dev += (uint64_t)(dev * dev);
        }
    }
    uint32_t variance = (uint32_t)(sq_dev / valid_count);
    /* Integer sqrt good enough for our purposes. */
    uint32_t std = 0;
    for (uint32_t g = 1 << 15; g > 0; g >>= 1) {
        uint32_t t = std | g;
        if (t * t <= variance) std = t;
    }
    uint32_t cv_pct = (std * 100) / mean;
    if (cv_pct > RHYTHM_CV_PCT_MAX) return false;
...
/* v4.14.18: sensor-presence gate rewritten.
 *
 * Old approach (v4.13.1 - v4.14.17): raw ADC p-p threshold over a
 * 500ms window. Easily fooled by USB hash, mains pickup, or any
 * single noise spike, because a single spike = large p-p. False
 * positive rate was unacceptable on noisy power.
 *
 * New approach: two-stage gate.
 *   Stage 1 (hard-disconnect): DC rail check. If ADC mean is outside
 *     [300, 3900], the line is floating or shorted — definitely no
 *     sensor. This is reliable and fast.
 *   Stage 2 (rhythmicity): if DC looks OK, require the beat detector
 *     to have produced a plausible rhythm recently. See the rhythm
 *     ring above — need ≥4 of last 8 IBIs in range [333,1500]ms
 *     (40-180 BPM) with CV ≤ 40%. This is the discriminator: noise
 *     can't maintain a heart-like rhythm for 4+ beats.
 *
 * Returns true if sensor appears present. Called from ppg_detect
 * which passes the current timestamp for staleness checks. */
static bool ppg_sensor_looks_present(uint32_t now_ms) {
    /* v4.14.14: warm-up — return false until we have ADC stats. */
    if (adc_stats_count < 5) return false;

    /* Stage 1: DC rail check. */
    uint32_t sum = 0;
    for (int i = 0; i < adc_stats_count; i++) sum += adc_stats_ring[i];
    uint16_t mean = (uint16_t)(sum / adc_stats_count);
    if (mean < PPG_RAW_DC_MIN || mean > PPG_RAW_DC_MAX) return false;

    /* Stage 2 (v4.14.19): bandpass amplitude floor. Real PPG drives
     * ma_beat into the hundreds; noise-triggered ghost beats rarely
     * reach PPG_AMP_MIN. This filters out the case where rhythmicity
     * happens to look plausible (e.g. mains coupling produces quasi-
     * periodic envelope modulation) but amplitude is tiny. */
    if (ppg_last_ma_beat < PPG_AMP_MIN) return false;

    /* Stage 3: rhythmicity. Beat detector must be producing a real
     * heart-like pattern. */
    return ppg_rhythm_looks_real(now_ms);
}
...
static bool ppg_detect(float filtered, uint32_t time_ms) {
    ...
    ppg_last_ma_beat = ma_beat;  /* v4.14.19: expose for gate check */

    /* Warmup — suppress detection until MAs have settled */
    if (ppg_sample_count < PPG_WARMUP_SAMPLES) return false;

    /* v4.14.18: gate check moved below detection. The detector now
     * ALWAYS runs, so it can discover beats even when the gate hasn't
     * yet been promoted to OK. Any beats found feed the rhythmicity
     * ring, which is how the gate decides whether the signal is real
     * vs noise-triggered chaos.
     *
     * If the gate is NOT OK, the beat is still tracked (for rhythmicity
     * evaluation) but NOT pushed to the coherence ring and NOT emitted
     * as a "confirmed beat" out of this function. Caller will see false.
     *
     * Hard-disconnect check first: if DC rail is obviously out of
     * range, skip even the detector work. This is the only case where
     * we short-circuit — floating/shorted line produces no useful
     * beat data anyway. */
    if (adc_stats_count >= 5) {
        uint32_t dc_sum = 0;
        for (int i = 0; i < adc_stats_count; i++) dc_sum += adc_stats_ring[i];
        uint16_t dc_mean = (uint16_t)(dc_sum / adc_stats_count);
        if (dc_mean < PPG_RAW_DC_MIN || dc_mean > PPG_RAW_DC_MAX) {
            /* Rail disconnect. Clear detector + rhythm + coherence. */
            if (g_sensor_gate_ok) {
                ppg_in_block = false;
                ppg_above_run = 0;
                ppg_last_beat_ms = 0;
                ...
                ppg_rhythm_clear();
                coh_clear();
                ble_log("gate: sensor disconnected (DC rail)");
        ...
        if (block_width_ms >= min_block_ms && since_last > refractory_ms) {
            ...
            if (ppg_last_beat_ms > 0) {
                ...
                if (ibi >= 300 && ibi <= 2000) {
                    ...
                    /* v4.14.18: push every in-range IBI to the rhythm
                     * ring — whether gate is currently OK or not. This
                     * is how the gate EVER gets promoted to OK in the
                     * first place: beats must accumulate and look
                     * heart-like before we call sensor "present". */
                    ppg_rhythm_push((uint16_t)ibi, ppg_block_peak_ms);
                }
            }
    ...
    /* v4.14.18: final gate decision. Now that this sample's detector
     * work is done (and any resulting beat is in the rhythm ring),
     * evaluate rhythmicity to decide sensor presence. */
    bool gate_now = ppg_sensor_looks_present(time_ms);
    if (!gate_now) {
        if (g_sensor_gate_ok) {
            /* Gate just flipped OK → not OK (lost rhythm). Clear
             * downstream state so reconnection is clean. Rhythm ring
             * is NOT cleared — we want it to remember recent beats so
             * a momentary single-beat gap doesn't lose all history. */
            ppg_in_block = false;
            ppg_above_run = 0;
            ppg_last_beat_ms = 0;
            ppg_current_ibi_ms = 0;
            ppg_current_bpm = 0;
            coh_clear();
            ble_log("gate: sensor lost (no rhythm)");
        }
        g_sensor_gate_ok = false;
        return false;
    }
    if (!g_sensor_gate_ok) {
        ...
        ble_log("gate: sensor present (rhythm confirmed)");
    }
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.19 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.19 started @ %d Hz (GPIO%d)",
