/*******************************************************************************
 * Smart Glasses Firmware v4.5
 * 
 * CRITICAL FIX: Restored differential AC drive for electrochromic lens health.
 * 
 * FEATURES:
 * - AC differential drive: GPIO26/GPIO27 alternate every 10ms (50Hz AC)
 * - 10Hz strobe: 100ms period (50ms dark, 50ms clear) at boot
 ...
 * 
 * CHANGELOG v4.5:
 * - RESTORED: Differential AC drive (PWM1/PWM2 alternating) - CRITICAL FIX
 * - Changed strobe to 10Hz (was 33Hz in v4.4)
 * - Removed breathing mode (not needed for current use case)
 * - Added power saving: -12dBm TX, slower advertising
 * - Strobe modulates effective_duty while AC task runs continuously
 ******************************************************************************/
 ...
 * Strobe: 10Hz = 100ms period = 50ms dark + 50ms clear
 ******************************************************************************/
#define AC_PERIOD_TICKS         1       /* 10ms = 50Hz AC alternation */
#define STROBE_HALF_PERIOD_AC   5       /* 5 AC cycles = 50ms per strobe phase */
...
static void led_task(void *param) {
    ...
    ESP_LOGI(TAG, "LED task started - 10Hz strobe");
    ...
    while (1) {
        ...
        /* Toggle strobe phase every 5 AC cycles (50ms) = 10Hz strobe */
        ...
        if (xTaskGetTickCount() - last_log_tick >= pdMS_TO_TICKS(30000)) {
            ...
            ESP_LOGI(TAG, "Strobe 10Hz @ %d%%, %lu sec remaining", 
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  Strobe: 10Hz @ %d%% brightness", DEFAULT_BRIGHTNESS);
