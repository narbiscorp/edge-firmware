/*******************************************************************************
 * Smart Glasses Firmware v4.11.0
 * 
 ...
 *
 * BLE ADVERTISING AUTO-OFF (v4.11.0):
 *   To minimize idle current when the device is used standalone (hall-only,
 *   no app), advertising is automatically disabled after BLE_IDLE_TIMEOUT_MS
 *   of no connection. Expected usage: user opens arms (boots/wakes), has a
 *   60-second window to connect from the app. If no connection occurs in
 *   that window, advertising stops and the device continues running its
 *   current hall-selected program with the radio off.
 *
 *   Re-arming: any hall event (tap or hold) resets the 60s window and
 *   restarts advertising if it was stopped. So to connect after the window
 *   has expired, the user just taps the magnet once — advertising comes
 *   back on for another 60s. A long close (>=5s) still enters deep sleep
 *   normally; on wake, advertising starts fresh with a new 60s window.
 *
 ...
 *
 *   Idle current reduction: radio-off idle is expected to drop from ~15mA
 *   (advertising at -6dBm, 100-200ms interval) to ~11mA baseline.
 * 
 * FEATURES:
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.11.0"
static const char *TAG = "SG_v4.11.0";
...
/* BLE auto-off state (v4.11.0).
 * ble_adv_active: tracks whether advertising is currently running.
 * ble_idle_deadline_tick: FreeRTOS tick when advertising should stop if
 *   still not connected. 0 = deadline disabled (while connected, or
 *   after it has already been consumed and advertising is off). */
...
/* Forward declarations for BLE auto-off helpers (v4.11.0).
 * Defined after adv_params so they can reference it. Declared here so
 * hall_task (which lives above adv_params) can call them. */
...
/*******************************************************************************
 * BLE ADVERTISING AUTO-OFF HELPERS (v4.11.0)
 *
 ...
 * After BLE_IDLE_TIMEOUT_MS of no connection, we stop advertising to
 * save the PA burst current. Controller/stack stay up so re-arming is
 * instant (no init overhead).
 *
 * ble_adv_rearm(): start advertising if not already running; reset deadline.
 *   Called on: boot, wake from deep sleep (via boot path), hall events,
 *   disconnect events.
 *
 * ble_adv_reset_deadline(): just push the deadline out another window.
 *   Used when advertising is already running and we want to give the user
 *   more time — currently called from ble_adv_rearm() itself.
 *
 * The actual stop happens in app_main's 1Hz loop when deadline elapses,
...
static void ble_adv_rearm(void) {
    ...
    if (!ble_adv_active) {
        ...
        if (err == ESP_OK) {
            ...
            ESP_LOGW(TAG, "BLE re-arm failed: %s", esp_err_to_name(err));
            /* Don't set ble_adv_active — leave deadline disabled so we don't
             * keep trying to stop something that never started. */
            return;
        }
    }
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  BLE auto-off: adv stops after %ds no connect (v4.11.0)",
    ...
    while (1) {
        ...
        /* v4.11.0: BLE idle auto-off.
         * If advertising is running, no client has connected, we're not in
         * OTA, and the deadline has elapsed — stop advertising to drop the
         * ~4mA of PA burst current from v4.9.12 advertising config.
         * ble_adv_active is cleared asynchronously by ADV_STOP_COMPLETE_EVT.
         * Deadline of 0 means "disabled" (set on connect, or after stop). */
        if (ble_adv_active && !is_connected && !in_ota_mode &&
            ble_idle_deadline_tick != 0 &&
            xTaskGetTickCount() >= ble_idle_deadline_tick) {
            ESP_LOGI(TAG, "BLE idle timeout (%ds no connect) — stopping advertising",
                     BLE_IDLE_TIMEOUT_MS / 1000);
