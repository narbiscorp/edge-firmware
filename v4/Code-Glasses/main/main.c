 ...
 * 
 * FEATURES:
 * - AC differential drive: GPIO26/GPIO27 via unified 1ms timer (100Hz AC)
 * - Three LED modes: STROBE, STATIC, BREATHE
 * - Unified timer: AC + strobe in single 1ms callback, sub-Hz to 50Hz
 ...
 * 
 * CHANGELOG v4.9.7:
 * - Unified 1ms esp_timer replaces separate AC drive task + strobe timer
 * - AC alternation at 100Hz, strobe counting in same callback
 * - Eliminates beat-frequency jitter between AC and strobe timing
 * - Default strobe 10Hz, all else unchanged from v4.9.6
 ******************************************************************************/
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.9.7"
static const char *TAG = "SG_v4.9.7";
 ...
 * AC + strobe: unified 1ms esp_timer (100Hz AC, strobe counting in same cb)
...
/* Unified drive timer state (1ms periodic — handles AC alternation + strobe) */
#define AC_HALF_PERIOD_MS  5   /* 5ms = 100Hz AC alternation */
static esp_timer_handle_t unified_timer = NULL;
static volatile uint32_t ac_tick = 0;
static volatile uint8_t ac_phase = 0;
static volatile uint32_t strobe_tick = 0;
...
/*******************************************************************************
 * UNIFIED DRIVE TIMER (1ms periodic esp_timer)
 * 
 * Single callback handles BOTH AC electrode alternation AND strobe phase
 * counting. Eliminates beat-frequency artifacts between separate AC task
 * and strobe timer. Strobe edges are always synchronized with AC transitions.
 * 
 * AC: toggle electrodes every AC_HALF_PERIOD_MS ticks (5ms = 100Hz)
 * Strobe: count 1ms ticks, toggle dark/clear at period boundary
 * Breathing/static: unified timer reads effective_duty set by led_task
 ******************************************************************************/
static void unified_timer_cb(void *arg) {
    /* ── OTA / session override ── */
    if (in_ota_mode || !session_active) {
        effective_duty = 0;
    }

    /* ── Strobe phase (only in strobe mode + active session) ── */
    if (led_mode == LED_MODE_STROBE && session_active && !in_ota_mode) {
        uint32_t period_ticks = 10000 / strobe_dhz;  /* deci-Hz → 1ms ticks */
        uint32_t dark_ticks = period_ticks * strobe_duty_pct / 100;
        if (dark_ticks < 1) dark_ticks = 1;
        if (dark_ticks >= period_ticks) dark_ticks = period_ticks - 1;

        strobe_tick++;
        if (strobe_tick >= period_ticks) strobe_tick = 0;

        effective_duty = (strobe_tick < dark_ticks) ? brightness : 0;
    }

    /* ── AC alternation ── */
    ac_tick++;
    if (ac_tick >= AC_HALF_PERIOD_MS) {
        ac_tick = 0;
        ac_phase = !ac_phase;
    }

    /* ── Apply PWM with AC phase ── */
    uint32_t raw = duty_to_raw(effective_duty);
    if (ac_phase == 0) {
        pwm1_set_raw(raw);
        pwm2_set_raw(1);
    } else {
...
static void strobe_stop(void) {
    /* Nothing to stop — unified timer keeps running for AC drive.
     * Strobe output stops because led_mode != LED_MODE_STROBE. */
}

static void strobe_update(void) {
    if (led_mode == LED_MODE_STROBE && session_active) {
        strobe_start();  /* reset tick counter */
    }
}

static void unified_timer_init(void) {
    esp_timer_create_args_t args = {
        .callback = unified_timer_cb,
        .name = "drive",
        .dispatch_method = ESP_TIMER_TASK,
    };
    ESP_ERROR_CHECK(esp_timer_create(&args, &unified_timer));
    ESP_ERROR_CHECK(esp_timer_start_periodic(unified_timer, 1000));  /* 1ms */
    ESP_LOGI(TAG, "Unified drive timer started (1ms / 100Hz AC)");
}
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  AC Drive: 100Hz unified timer (GPIO26/27)");
    ...
    /* Start unified drive timer (1ms periodic — AC alternation + strobe) */
    unified_timer_init();
