 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.12.6"
static const char *TAG = "SG_v4.12.6";
...
static void gatts_event_handler(esp_gatts_cb_event_t event, 
    ...
    switch (event) {
        ...
        case ESP_GATTS_CONNECT_EVT:
             ...
             *
             * v4.12.5: interval tightened 40-60ms → 7.5-15ms, latency
             * 4→0. With PPG streaming at 50Hz the previous 40ms interval
             * meant 2 samples per connection event — the BLE TX buffer
             * filled frequently (ble_err growing ~5/s in v4.12.4 on PC).
             * 15ms interval lets every sample ride its own connection
             * event; 7.5ms if the central honors the min. Latency=0 is
             * safe now that there's no pre-OTA concern. */
            esp_ble_conn_update_params_t conn_params = {
                .latency = 0,           /* No skipping — we want every event */
                .max_int = 0x0C,        /* 15ms max interval (was 60ms) */
                .min_int = 0x06,        /* 7.5ms min interval (was 40ms) */
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.12.6 started @ %d Hz (GPIO%d)",
