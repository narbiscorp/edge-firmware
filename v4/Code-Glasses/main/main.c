 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.12.1"
static const char *TAG = "SG_v4.12.1";
...
/* v4.12.0: PulseSensor analog input.
 * GPIO36 is the SENSOR_VP pin — input-only, mapped to ADC1_CHANNEL_0.
 * ADC1 is chosen (not ADC2) because ADC2 contends with WiFi, and even
 * though WiFi is off in this build, using ADC1 keeps things robust.
 * GPIO36 also avoids conflict with the PWM outputs (GPIO26/27) and the
 * hall sensor (GPIO4).
 *
 * Electrical: PulseSensor Vout swings around ~1.65V midrail, ±0.2–0.4V
 * on a good finger contact. ADC_ATTEN_DB_11 maps 0–3.1V to 0–4095, so
 * the quiescent DC sits near code 2050 and the signal occupies a few
 * hundred codes of swing. Plenty of headroom; no clipping concerns
 * under normal conditions. */
#define PPG_ADC_GPIO            GPIO_NUM_36
#define PPG_ADC_CHANNEL         ADC1_CHANNEL_0
 ...
 */
static const float PPG_BP_B[5] = { 0.13110644f,  0.0f, -0.26221288f,  0.0f,  0.13110644f };
static const float PPG_BP_A[5] = { 1.0f, -2.65445601f,  2.63289834f, -1.24832672f,  0.27221494f };
...
/* Oversampled ADC read — averages PPG_OVERSAMPLE consecutive reads.
 * Cheap antialiasing: each read is ~30µs, 8× = 240µs total (vs 20ms tick).
 * Averaging 8 reads gives ~3 bits of noise reduction on a 12-bit ADC.
 */
static inline uint16_t ppg_read_oversampled(void) {
    ...
    return (uint16_t)(acc / PPG_OVERSAMPLE);
}
...
static void ppg_task(void *arg) {
    ...
    uint32_t last_hb_ms = 0;   /* Heartbeat ble_log — confirms task is alive */

    ESP_LOGI(TAG, "PPG task started @ %d Hz (GPIO%d, ADC1_CH0)",
    ...
    while (1) {
        ...
        /* v4.12.1: drive LED_MODE_PULSE_ON_BEAT. Writing the pulse start
         * tick unconditionally per beat — led_task reads it and computes
         * the envelope. If current mode isn't PULSE_ON_BEAT, the write is
         * harmless (led_task just ignores it). */
        if (is_beat) {
        ...
        /* v4.12.1: emit ADC stats every 500ms. Dashboard uses these to
         * show raw sensor health independent of detection results. */
        if (now_ms - last_stats_ms >= 500) {
            last_stats_ms = now_ms;
            ppg_emit_adc_stats();
        }

        /* v4.12.1: periodic heartbeat ble_log (every 5s) — confirms the
         * task is alive and gives dashboard a pulse it can reference to
         * detect firmware freeze. Also logs the current MA_beat value
         * which is the best indicator of "is there ANY signal energy." */
        if (now_ms - last_hb_ms >= 5000) {
            last_hb_ms = now_ms;
            ble_log("ppg t=%us raw=%u MAb=%.1f beats=%u",
...
void app_main(void) {
     ...
     * Priority 3 ensures it preempts led_task and hall_task for timing
     * accuracy, but yields to ota_task during flash writes. Stack 4096
     * — filter arrays are globals so the stack only holds locals. */
    xTaskCreate(ppg_task, "ppg_task", 4096, NULL, 3, &ppg_task_handle);
