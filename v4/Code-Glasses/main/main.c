/*******************************************************************************
 * Smart Glasses Firmware v4.6
 * 
 * Based on v4.5 (differential AC drive restore). Changed strobe to 1Hz.
 * 
 * FEATURES:
 * - AC differential drive: GPIO26/GPIO27 alternate every 10ms (50Hz AC)
 * - 1Hz strobe: 1000ms period (500ms dark, 500ms clear) at boot
 ...
 * 
 * CHANGELOG v4.6:
 * - Changed strobe from 10Hz to 1Hz (500ms dark, 500ms clear)
 * - All other functionality unchanged from v4.5
 ******************************************************************************/
 ...
 * Strobe: 1Hz = 1000ms period = 500ms dark + 500ms clear
 ******************************************************************************/
#define AC_PERIOD_TICKS         1       /* 10ms = 50Hz AC alternation */
#define STROBE_HALF_PERIOD_AC   50      /* 50 AC cycles = 500ms per strobe phase */
...
static void led_task(void *param) {
    ...
    ESP_LOGI(TAG, "LED task started - 1Hz strobe");
    ...
    while (1) {
        ...
        /* Toggle strobe phase every 50 AC cycles (500ms) = 1Hz strobe */
        ...
        if (xTaskGetTickCount() - last_log_tick >= pdMS_TO_TICKS(30000)) {
            ...
            ESP_LOGI(TAG, "Strobe 1Hz @ %d%%, %lu sec remaining", 
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  Strobe: 1Hz @ %d%% brightness", DEFAULT_BRIGHTNESS);
