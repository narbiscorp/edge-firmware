/*******************************************************************************
 * Smart Glasses Firmware v4.14.18
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.18"
static const char *TAG = "SG_v4.14.18";
...
static bool ppg_sensor_looks_present(uint32_t now_ms) {
    ...
    /* Stage 2: rhythmicity. The beat detector has been running in
     * parallel — if real pulses are coming through, it has beats in
     * its ring. If only noise, the ring is either empty or chaotic. */
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.18 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.18 started @ %d Hz (GPIO%d)",
