/*******************************************************************************
 * Smart Glasses Firmware v4.14.7
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.7"
static const char *TAG = "SG_v4.14.7";
...
#define INDICATOR_PRE_DELAY_MS  1000   /* v4.14.6: wait this long after the
...
static void indicator_trigger(uint8_t count, uint32_t hold_ms) {
    ...
    indicator.hold_until_tick = 0;
    if (hold_ms > 0) {
        /* Hold ends: delay + (count × pulse) + hold. */
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.7 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.7 started @ %d Hz (GPIO%d)",
