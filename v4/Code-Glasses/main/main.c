/*******************************************************************************
 * Smart Glasses Firmware v4.14.26
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.14.26"
static const char *TAG = "SG_v4.14.26";
...
static void led_task(void *param) {
    ...
    while (1) {
         ...
         *******************************************************************/
            ...
            /* v4.14.26: apply difficulty-dependent knee points. Remap s
             * from [coh_start, coh_end] → [0, 100] before converting to
             * duty. Below coh_start: effective_s=0 (lens stays dark).
             * Above coh_end: effective_s=100 (lens fully clear). Between:
             * linear interpolation.
             *
             *   Easy: start=0, end=100 → identity map (current behavior)
             *   Harder levels: both knees move up so the user must earn
             *   higher coherence to see any lens response. */
            uint8_t diff = coh_difficulty;
            if (diff > 3) diff = 0;
            float coh_start = (float)coh_difficulty_table[diff].coh_start;
            float coh_end   = (float)coh_difficulty_table[diff].coh_end;
            float effective_s;
            if (s <= coh_start) {
                effective_s = 0.0f;
            } else if (s >= coh_end) {
                ...
                effective_s = (s - coh_start) * 100.0f / (coh_end - coh_start);
            }
    ...
    switch (cmd) {
        ...
        case 0xB8:  /* v4.14.25: Set coherence difficulty.
            ...
            if (arg <= 3) {
                ...
                ESP_LOGI(TAG, "BLE: coh difficulty = %d", arg);
...
/* v4.14.26: coherence difficulty (HeartMath-style).
 *
 * Philosophy change from v4.14.25: the raw coherence score stays the
 * same across all difficulty levels so it remains comparable across
 * sessions and against published HeartMath literature. Difficulty
 * only affects:
 *   (1) the COHERENCE_LENS opacity mapping curve (this file, led_task),
 *   (2) the display-zone thresholds on the dashboard (client-side).
 *
 * Lens curve knee points — the lens stays fully dark until coherence
 * reaches coh_start, then linearly clears through to coh_end (fully
 * clear). At higher difficulty, both knees move up, requiring a higher
 * coherence to see lens response:
 *
 *   Easy   (0): start  0, end 100  — classic linear (same as v4.14.24)
 *   Medium (1): start 30, end  70  — must earn "medium" to see change
 *   Hard   (2): start 50, end  85  — must earn "high" to clear
 *   Expert (3): start 65, end  95  — must sustain near-peak coherence
 *
 * Runtime-settable via BLE command 0xB8. */
typedef struct {
    uint8_t coh_start;   /* coherence below this → lens fully dark */
    uint8_t coh_end;     /* coherence at/above this → lens fully clear */
} coh_difficulty_t;

static const coh_difficulty_t coh_difficulty_table[4] = {
    {  0, 100 },   /* Easy   — linear map, historical behavior */
    { 30,  70 },   /* Medium */
    { 50,  85 },   /* Hard   */
    { 65,  95 },   /* Expert */
};
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.14.26 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.14.26 started @ %d Hz (GPIO%d)",
