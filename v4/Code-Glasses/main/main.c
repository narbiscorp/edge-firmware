 ...
 * 
 * FEATURES:
 * - AC differential drive: GPIO26/GPIO27 via gptimer 100µs ISR (1kHz AC)
 ...
 * 
 * CHANGELOG v4.9.9:
 * - AC frequency raised from 100Hz to 1kHz (AC_HALF_TICKS 50→5) to eliminate
 *   beat-frequency artifacts at strobe rates above ~20Hz
 * - Cached strobe dark threshold to avoid multiply+divide every ISR tick
 * - All else unchanged from v4.9.8
 ******************************************************************************/
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.9.9"
static const char *TAG = "SG_v4.9.9";
...
#define AC_HALF_TICKS           5       /* 5 × 100µs = 0.5ms = 1kHz AC */
 ...
 * 
 * AC: toggle every AC_HALF_TICKS (5 × 100µs = 0.5ms = 1kHz)
...
static void drive_timer_init(void) {
    ...
    ESP_LOGI(TAG, "Drive timer started (100µs gptimer ISR / 1kHz AC)");
}
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  AC Drive: 1kHz gptimer ISR (GPIO26/27)");
