/*******************************************************************************
 * Smart Glasses Firmware v4.12.0-demo  (DEMO / BOOTH VARIANT)
 *
 * Same as v4.12.0 but with hall-sensor input and session auto-sleep disabled
 * so the glasses keep running indefinitely for unattended demos. Everything
 * else — LED modes, BLE control, OTA, pulse sensor stream on 0xFF04 —
 * behaves identically.
 *
 * DIFFERENCES FROM v4.12.0:
 *   - Hall-gesture task is NOT created. Magnet taps are ignored (no program
 *     cycling), and the 5-second magnet-hold deep-sleep is disabled. If the
 *     glasses were originally put to sleep via magnet, they'll still wake on
 *     ext0 LOW like before — the wake path is untouched. Once awake, the
 *     magnet does nothing.
 *   - Session duration defaulted to effectively never (24 hours). The 0xA4
 *     BLE command can still shorten it at runtime, and the 0xA7 sleep command
 *     still works for an explicit software shutdown.
 *   - Main loop no longer calls enter_deep_sleep() when led_task exits —
 *     instead it restarts led_task with a fresh long session. This means a
 *     "session end" only happens if the user explicitly requests it via BLE.
 *   - BLE idle auto-off (the 60s-no-connect → radio teardown path) is
 *     also disabled in this build. In the regular build, a magnet tap
 *     re-arms BLE after teardown; since hall input is off in this build,
 *     tearing down BLE would strand the device unreachable until reboot.
 *     Always-on BLE is the right default for a booth / demo use case.
 *
 * INTENDED USE:
 *   Trade show / demo booth / long-running HRV data capture sessions where
 *   you can't have the glasses nodding off mid-pitch because someone waved
 *   a magnet near them or the 10-minute session timer ran out.
 *
 * SAFETY NOTE:
 *   Because the magnet can't put these to sleep, the only way to shut them
 *   down from the outside is (a) send 0xA7 via BLE, or (b) remove power.
 *   Battery will drain until flat. Plan accordingly.
 *
 ******************************************************************************/
 ...
 *
 * Adds HRV pulse sensor streaming alongside all existing lens/BLE/OTA features.
 * Pulse sensor (PulseSensor.com analog) wired to GPIO35 (ADC1_CH7, pin 11 on
 * the ESP32-PICO-D4, a.k.a. VDET_2). Sampled at 200Hz, raw 12-bit ADC streamed
 * over BLE at 50Hz via a new NOTIFY characteristic (0xFF04) in 12-byte packets
 * matching the Narbis Edge HRV Sensor v4 firmware protocol. All HRV math
 * (filtering, peak detection, RR intervals, RMSSD etc.) runs in the webapp.
 *
 * BLE GATT LAYOUT (v4.12.0):
 *   Service 0x00FF
 *     0xFF01  Control  (WRITE)   — lens/strobe/breathe/OTA commands (unchanged)
 *     0xFF02  OTA data (WRITE)   — OTA page payload (unchanged)
 *     0xFF03  Status   (NOTIFY)  — OTA status/progress (unchanged)
 *     0xFF04  Pulse    (NOTIFY)  — 12-byte raw ADC packets @ 50Hz (NEW)
 *
 * PULSE PACKET FORMAT (12 bytes, matches .ino exactly):
 *   [0]     uint8   0x01 packet type
 *   [1-2]   uint16  raw ADC (LE, 12-bit 0-4095)
 *   [3-4]   uint16  sample index (LE, wraps 65535)
 *   [5-8]   uint32  timestamp ms (LE, from esp_timer_get_time/1000)
 *   [9-10]  uint16  raw ADC min over last ~200 samples (LE) — debug
 *   [11]    uint8   reserved
 *
 * A short name note: the device still advertises as "Narbis_Edge" (not
 * "Narbis Edge HRV Sensor" as the .ino does), so the existing controller
 * webapp / OTA tool continue to discover the glasses by their familiar name.
 * The HRV dashboard webapp needs a one-line patch to match (name filter
 * + CHR UUID). See Devon's paired HRV webapp patch for v4.12.0.
 *
 * PULSE SAMPLING TASK:
 *   - 200Hz sampling via vTaskDelay(5ms) loop (matches .ino's 5000µs period).
 *   - ADC: oneshot driver, 12-bit width, 11dB attenuation (0-3.3V range).
 *   - Every 4th sample is sent as a BLE notify → 50Hz stream over the air.
 *   - Sampling runs continuously whenever BLE stack is up, regardless of
 *     lens session state (you may want HRV without an active session).
 *   - Paused during OTA (to keep the radio free for flash writes).
 *   - Paused while BLE stack is torn down (no point sampling with nowhere
 *     to send). Resumes on any BLE re-arm via hall gesture.
 *
 * CHANGELOG v4.12.0:
 * - Added pulse sensor ADC sampling on GPIO35 / ADC1_CH7 (pin 11 of the
 *   ESP32-PICO-D4). 200Hz polling, 50Hz BLE notify.
 * - Added 0xFF04 NOTIFY characteristic with its own CCCD. Independent of
 *   the existing 0xFF03 status CCCD — client can subscribe to pulse
 *   without enabling OTA status notifications and vice versa.
 * - GATTS_NUM_HANDLE bumped 10 -> 13 to cover the new char + CCCD
 *   (service + 4 chars + 4 value handles + 2 CCCDs).
 * - pulse_task added at priority 3 (between hall_task @2 and ota_task @5)
 *   so it preempts the 10ms led_task but never preempts OTA work.
 * - Pulse notifications gate on (pulse_notify_enabled && is_connected
 *   && !in_ota_mode && ble_stack_up), so all the existing auto-off and
 *   OTA pause logic continues to work without special cases.
 * - Requires esp_adc component in main/CMakeLists.txt REQUIRES list.
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.12.0-demo"
static const char *TAG = "SG_v4.12.0d";
...
#define GATTS_CHAR_UUID_PULSE   0xFF04  /* v4.12.0: Pulse sensor raw ADC stream */
#define GATTS_NUM_HANDLE        13      /* Service + 4 chars + value handles + 2 CCCDs */
...
/* Pulse sensor ADC (v4.12.0):
 * Wired to pin 11 of the ESP32-PICO-D4 = VDET_2 = GPIO35 = ADC1_CH7.
 * GPIO35 is input-only — no pullups/pulldowns — which is fine for an
 * analog source driving a voltage into it. 6dB attenuation gives a
 * usable input range of roughly 0-2.2V. We use 6dB (narrower than 12dB)
 * because the PulseSensor's output sits near mid-rail (~1.65V) with
 * excursions well within 0-2.2V. The narrower range gives ~1.4x more
 * effective resolution on the signal of interest. If the sensor ever
 * drifts to the rail on a given setup, it'll still clip at 2.2V — but
 * that's an indicator of a hardware issue, not something to mask with
 * more ADC headroom. 12-bit width gives 0-4095 raw counts. */
#define PULSE_ADC_UNIT          ADC_UNIT_1
#define PULSE_ADC_CHANNEL       ADC_CHANNEL_7       /* GPIO35 */
#define PULSE_SAMPLE_HZ         200
#define PULSE_SAMPLE_MS         (1000 / PULSE_SAMPLE_HZ)  /* 5ms */
#define PULSE_BLE_DECIMATION    4                    /* 200Hz / 4 = 50Hz BLE notify */
#define PULSE_ADC_RANGE_WINDOW  200                  /* Samples for debug min tracking */
...
#define DEFAULT_SESSION_MIN     1440    /* 24 hour session (demo: effectively never auto-end) */
...
static uint16_t pulse_char_handle = 0;     /* v4.12.0 */
static uint16_t pulse_cccd_handle = 0;     /* v4.12.0 */
static bool notifications_enabled = false;  /* OTA status char (0xFF03) CCCD */
static bool pulse_notify_enabled = false;   /* v4.12.0: pulse char (0xFF04) CCCD */
static bool is_connected = false;

/* Pulse sensor state (v4.12.0) */
static adc_oneshot_unit_handle_t pulse_adc_handle = NULL;
static volatile uint16_t pulse_sample_idx = 0;      /* Wraps at 65535 */
static volatile uint32_t pulse_sample_cnt = 0;      /* Debug total */
static volatile uint32_t pulse_tx_cnt = 0;          /* Debug BLE notify count */
static volatile uint16_t pulse_adc_min = 4095;      /* Debug range window */
static volatile uint16_t pulse_adc_max = 0;         /* Debug range window */
...
/*******************************************************************************
 * PULSE SENSOR (v4.12.0)
 *
 * Samples an analog PulseSensor.com on GPIO35 (ADC1_CH7, pin 11 of the
 * ESP32-PICO-D4) at 200Hz. Every 4th sample is streamed over BLE via a
 * 12-byte notify packet on 0xFF04 (50Hz air rate). The protocol matches
 * the standalone Narbis Edge HRV Sensor v4 firmware so the HRV dashboard
 * webapp can parse the packets identically.
 *
 * All filtering, peak detection, RR calculation, and HRV math is done in
 * the webapp — firmware only forwards raw ADC counts.
 *
 * Gates: samples continuously whenever BLE is up and no OTA is in progress.
 * Notifies only when the central has subscribed to the 0xFF04 CCCD.
 ******************************************************************************/
static void pulse_adc_init(void) {
    /* One-shot ADC1 unit (shared pattern is fine for a single channel). */
    adc_oneshot_unit_init_cfg_t unit_cfg = {
        .unit_id = PULSE_ADC_UNIT,
        .ulp_mode = ADC_ULP_MODE_DISABLE,
    };
    ESP_ERROR_CHECK(adc_oneshot_new_unit(&unit_cfg, &pulse_adc_handle));

    /* 12-bit width + 11dB attenuation = matches .ino:
     *   analogReadResolution(12);
     *   analogSetAttenuation(ADC_11db);
     * ADC_BITWIDTH_DEFAULT maps to ADC_BITWIDTH_12 on ESP32. */
    adc_oneshot_chan_cfg_t chan_cfg = {
        .bitwidth = ADC_BITWIDTH_DEFAULT,
        .atten = ADC_ATTEN_DB_6,    /* ~0-2.2V range — tighter than 12dB for better resolution on PPG mid-rail signal */
    };
    ESP_ERROR_CHECK(adc_oneshot_config_channel(pulse_adc_handle,
                                               PULSE_ADC_CHANNEL, &chan_cfg));

    /* Brief self-test sweep so the boot log shows whether the sensor is
     * wired up. Matches the .ino's [INIT] ADC test block. */
    ESP_LOGI(TAG, "Pulse ADC init: GPIO35 / ADC1_CH7, 12-bit, 6dB atten (0-2.2V)");
    for (int i = 0; i < 5; i++) {
        int raw = 0;
        adc_oneshot_read(pulse_adc_handle, PULSE_ADC_CHANNEL, &raw);
        ESP_LOGI(TAG, "  pulse sample %d: %d (%.2fV)",
                 i, raw, raw * 2.2f / 4095.0f);
        vTaskDelay(pdMS_TO_TICKS(50));
    }
}

/* Build and send a 12-byte pulse packet (LE byte order, matches .ino). */
static void pulse_send_packet(uint16_t raw, uint16_t idx, uint32_t ts_ms,
                              uint16_t adc_min) {
    if (!pulse_notify_enabled || !is_connected) return;
    if (!ble_stack_up || gatts_if_global == ESP_GATT_IF_NONE) return;
    if (pulse_char_handle == 0) return;
    ...
    pkt[5]  = (uint8_t)(ts_ms & 0xFF);
    pkt[6]  = (uint8_t)((ts_ms >> 8) & 0xFF);
    pkt[7]  = (uint8_t)((ts_ms >> 16) & 0xFF);
    pkt[8]  = (uint8_t)((ts_ms >> 24) & 0xFF);
    pkt[9]  = (uint8_t)(adc_min & 0xFF);
    pkt[10] = (uint8_t)((adc_min >> 8) & 0xFF);
    pkt[11] = 0;

    /* need_confirm=false => notify (not indicate). Same call pattern used
     * elsewhere in this file for OTA status notifications. */
    esp_ble_gatts_send_indicate(gatts_if_global, conn_id_global,
                                pulse_char_handle, sizeof(pkt), pkt, false);
    pulse_tx_cnt++;
}

/* v4.12.0 fix: Because CONFIG_FREERTOS_HZ=100 (1 tick = 10ms), we can't
 * use vTaskDelayUntil(5ms) — pdMS_TO_TICKS(5) rounds down to zero ticks
 * and trips the xTimeIncrement>0 assert in tasks.c:1499.
 * Instead, drive sampling from a periodic esp_timer callback (runs at
 * true 5ms precision regardless of tick rate). The timer notifies the
 * pulse_task, which does the actual ADC read + BLE notify work. Keeping
 * the work in a task (vs. doing it in the timer callback directly) is
 * the recommended pattern — esp_timer callbacks run in a shared timer
 * task that other code may depend on, and adc_oneshot_read uses a mutex
 * that is not ISR-safe. */
static esp_timer_handle_t pulse_sample_timer = NULL;
static TaskHandle_t pulse_task_handle = NULL;

static void pulse_timer_cb(void *arg) {
    /* Called in esp_timer task context (ESP_TIMER_TASK dispatch), NOT ISR.
     * Use the task-context variant of the notify API. Keep this callback
     * short — esp_timer callbacks are serialized, so any delay here stalls
     * every other task-dispatch timer in the system. */
    if (pulse_task_handle) {
        xTaskNotifyGive(pulse_task_handle);
    }
}

static void pulse_task(void *param) {
    uint8_t decim_ctr = 0;
    uint32_t window_samples = 0;
    int64_t last_stat_us = 0;

    ESP_LOGI(TAG, "Pulse task started (%dHz sampling, %dHz notify)",
             PULSE_SAMPLE_HZ, PULSE_SAMPLE_HZ / PULSE_BLE_DECIMATION);

    /* Create the periodic sample timer. 5000µs = 200Hz. */
    const esp_timer_create_args_t targs = {
        .callback = &pulse_timer_cb,
        .name = "pulse_tick",
        .dispatch_method = ESP_TIMER_TASK,
    };
    ESP_ERROR_CHECK(esp_timer_create(&targs, &pulse_sample_timer));
    ESP_ERROR_CHECK(esp_timer_start_periodic(pulse_sample_timer,
                                             1000ULL * PULSE_SAMPLE_MS));

    while (1) {
        /* Wait for the 5ms tick from pulse_timer_cb. Timeout of 100ms is
         * a paranoid upper bound — if the timer stops firing for any
         * reason, we still loop and re-check gates rather than block
         * forever. */
        ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(100));

        /* Skip sampling entirely while OTA is running. We want every
         * ounce of BLE throughput and flash-write headroom during OTA. */
        if (in_ota_mode) {
            decim_ctr = 0;
            continue;
        }

        /* Skip if BLE stack is down (the adv auto-off path).
         * Resumes automatically when a hall gesture re-arms BLE. */
        if (!ble_stack_up) {
            decim_ctr = 0;
            continue;
        }
        ...
        int raw_i = 0;
        esp_err_t err = adc_oneshot_read(pulse_adc_handle,
                                         PULSE_ADC_CHANNEL, &raw_i);
        if (err != ESP_OK) continue;
        uint16_t raw = (uint16_t)(raw_i & 0x0FFF);   /* 12-bit clamp */

        pulse_sample_cnt++;
        window_samples++;

        /* Track raw min/max over a rolling ~PULSE_ADC_RANGE_WINDOW samples
         * for the debug field (matches .ino's adcMin tracking). */
        if (raw < pulse_adc_min) pulse_adc_min = raw;
        if (raw > pulse_adc_max) pulse_adc_max = raw;

        /* Decimate 4:1 → 50Hz BLE stream. */
        decim_ctr++;
        if (decim_ctr >= PULSE_BLE_DECIMATION) {
            decim_ctr = 0;
            uint32_t ts_ms = (uint32_t)(esp_timer_get_time() / 1000ULL);
            pulse_send_packet(raw, pulse_sample_idx, ts_ms, pulse_adc_min);
            pulse_sample_idx++;
        }

        /* Reset range window every PULSE_ADC_RANGE_WINDOW samples
         * (~1 second at 200Hz). Matches the .ino's 5s debug cadence
         * scaled for tighter tracking. */
        if (window_samples >= PULSE_ADC_RANGE_WINDOW) {
            window_samples = 0;
            pulse_adc_min = 4095;
            pulse_adc_max = 0;
        }

        /* Periodic debug log (~every 5s, matches .ino). Uses µs clock so
         * cadence is independent of FreeRTOS tick rate. */
        int64_t now_us = esp_timer_get_time();
        if (now_us - last_stat_us >= 5000000LL) {
            last_stat_us = now_us;
            ESP_LOGI(TAG, "Pulse: samples=%lu tx=%lu sub=%s",
                     (unsigned long)pulse_sample_cnt,
                     (unsigned long)pulse_tx_cnt,
                     pulse_notify_enabled ? "Y" : "N");
        }
    }
}
...
static esp_err_t ble_stack_teardown(void) {
    ...
    cccd_handle = 0;
    pulse_char_handle = 0;          /* v4.12.0 */
    pulse_cccd_handle = 0;          /* v4.12.0 */
    service_handle = 0;
    notifications_enabled = false;
    pulse_notify_enabled = false;   /* v4.12.0 */
    ...
    switch (event) {
        ...
        case ESP_GATTS_ADD_CHAR_EVT: {
            ...
            if (char_uuid == GATTS_CHAR_UUID_CTRL) {
                ...
                /* Add CCCD for status notifications (0xFF03) */
            ...
            else if (char_uuid == GATTS_CHAR_UUID_PULSE) {
                /* v4.12.0: Pulse characteristic added. Its CCCD comes next. */
                pulse_char_handle = param->add_char.attr_handle;
                ESP_LOGI(TAG, "Char added step 4, handle %d (pulse 0xFF04)",
                         pulse_char_handle);
        ...
        case ESP_GATTS_ADD_CHAR_DESCR_EVT: {
            /* Two CCCDs are added in sequence: first for the status char
             * (0xFF03), then for the pulse char (0xFF04). The ADD_CHAR_DESCR
             * event doesn't carry a useful discriminator, so we branch on
             * whichever handle hasn't been filled in yet. After the first
             * CCCD is registered we also need to add the pulse char itself
             * (v4.12.0 — kicks the chain forward). */
            if (cccd_handle == 0) {
                cccd_handle = param->add_char_descr.attr_handle;
                ESP_LOGI(TAG, "Status CCCD handle %d", cccd_handle);

                /* Add pulse characteristic (0xFF04) with NOTIFY. */
                esp_bt_uuid_t pulse_uuid = {
                    .len = ESP_UUID_LEN_16,
                    .uuid = {.uuid16 = GATTS_CHAR_UUID_PULSE}
                };
                esp_ble_gatts_add_char(service_handle, &pulse_uuid,
                                       ESP_GATT_PERM_READ,
                                       ESP_GATT_CHAR_PROP_BIT_READ |
                                       ESP_GATT_CHAR_PROP_BIT_NOTIFY,
                                       NULL, NULL);
            } else {
                pulse_cccd_handle = param->add_char_descr.attr_handle;
                ESP_LOGI(TAG, "Pulse CCCD handle %d - BLE setup complete",
                         pulse_cccd_handle);
                ESP_LOGI(TAG, "Handles: ctrl=%d ota=%d status=%d pulse=%d",
                         ctrl_char_handle, ota_char_handle,
                         status_char_handle, pulse_char_handle);
                ESP_LOGI(TAG, "CCCDs: status=%d pulse=%d",
                         cccd_handle, pulse_cccd_handle);
        ...
        case ESP_GATTS_WRITE_EVT:
            ...
            else if (param->write.handle == pulse_cccd_handle) {
                /* v4.12.0: Pulse stream CCCD. Independent of status CCCD —
                 * client can subscribe to pulse only, or status only, or
                 * both, in any order. */
                if (param->write.len == 2) {
                    uint16_t cccd_value = param->write.value[0] | (param->write.value[1] << 8);
                    pulse_notify_enabled = (cccd_value == 0x0001);
                    ESP_LOGI(TAG, "Pulse notifications %s", pulse_notify_enabled ? "enabled" : "disabled");
                    if (pulse_notify_enabled) {
                        /* Reset counters so the webapp starts from a clean
                         * sample index — otherwise continuity across reconnects
                         * looks odd in debug traces. */
                        pulse_sample_idx = 0;
                        pulse_tx_cnt = 0;
                    }
                }
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "Smart Glasses v%s  *** DEMO VARIANT ***", FIRMWARE_VERSION);
    ESP_LOGI(TAG, "  Boot: Program 1 = BREATHE %d BPM %s @ %d%% brightness",
             breathe_bpm, breathe_wave == 0 ? "sine" : "linear", DEFAULT_BRIGHTNESS);
    ESP_LOGI(TAG, "  Hall: DISABLED (magnet input ignored, no hold-to-sleep)");
    ESP_LOGI(TAG, "  Programs: 1=BREATHE 2=BREATHE+STROBE 3=STROBE (via BLE only)");
    ESP_LOGI(TAG, "  Session: %d minutes (auto-restarts, never sleeps passively)",
             DEFAULT_SESSION_MIN);
    ...
    ESP_LOGI(TAG, "  BLE auto-off: DISABLED in demo build (always connectable)");
    ESP_LOGI(TAG, "  Pulse sensor (v4.12.0): GPIO35/ADC1_CH7, %dHz sample, %dHz BLE (0xFF04)",
             PULSE_SAMPLE_HZ, PULSE_SAMPLE_HZ / PULSE_BLE_DECIMATION);
    ...
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "DEMO BUILD: hall disabled, session auto-restarts");
    ESP_LOGI(TAG, "Only way to sleep is 0xA7 via BLE or power removal");
    ...
    ESP_LOGI(TAG, "HRV: subscribe to 0xFF04 for 50Hz raw pulse ADC stream");
    ...
    /* v4.12.0: Initialize pulse sensor ADC (GPIO35 / ADC1_CH7 / pin 11). */
    pulse_adc_init();
    ...
    /* DEMO VARIANT: hall-gesture task intentionally NOT created.
     * In the regular v4.12.0 build this line would be:
     *   xTaskCreate(hall_task, "hall_task", 2048, NULL, 2, NULL);
     * Skipping it disables both short-tap program advance and 5-second
     * hold-to-sleep — the magnet becomes inert for the life of this boot.
     * ble_adv_rearm() (normally called from the hall task on rising edge)
     * is no longer invoked, but the boot path has already armed adv once
     * and the disconnect path re-arms as needed, so advertising still
     * behaves correctly. */

    /* v4.12.0: Create pulse sensor task.
     * Priority 3 sits between hall_task (2) and ota_task (5): it preempts
     * the led_task 10ms loop briefly every 5ms for a single ADC read + an
     * occasional BLE notify, but never preempts OTA flash writes. Stack at
     * 3072 covers the ADC call + small local buffers with margin. */
    xTaskCreate(pulse_task, "pulse_task", 3072, NULL, 3, &pulse_task_handle);

    /* Main loop - monitor session state and BLE idle timeout.
     * DEMO VARIANT: if the session timer elapses (led_task exits), we
     * restart it with a fresh session instead of going to sleep. There is
     * no passive path to sleep in this build — only the explicit 0xA7 BLE
     * command, which sets session_active=false AND calls enter_deep_sleep()
     * inline (never returns), so we won't hit the restart branch for that. */
    while (1) {
        /* Demo: auto-restart session instead of sleeping */
        if (!session_active && led_task_handle == NULL && !in_ota_mode) {
            ESP_LOGI(TAG, "DEMO: session timer elapsed — restarting led_task");
            xTaskCreate(led_task, "led_ctrl", 4096, NULL, 1, &led_task_handle);
        }
         ...
         *
         * DEMO VARIANT: auto-off is DISABLED. Reasoning: in the regular
         * build, the user brings BLE back by tapping the magnet (hall task
         * calls ble_adv_rearm on the rising edge). Since hall input is
         * disabled in this build, tearing down BLE would strand the device
         * unreachable until reboot. For a booth / demo use case, always-on
         * BLE is almost certainly what you want. If you'd prefer auto-off
         * anyway, delete the `#if 0` guard below. */
#if 0  /* auto-off disabled in demo variant */
