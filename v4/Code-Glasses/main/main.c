...
static void pulse_task(void *param) {
    ...
    const TickType_t period = pdMS_TO_TICKS(PULSE_SAMPLE_MS);

    while (1) {
        /* Fixed-interval delay — vTaskDelayUntil keeps jitter to one tick
         * even if the task gets preempted. The LED/hall tasks are higher
         * priority but both do very short work, so 5ms is realistic. */
        ...
        /* Periodic debug log (~every 5s, matches .ino). */
...
void app_main(void) {
    ...
    xTaskCreate(pulse_task, "pulse_task", 3072, NULL, 3, NULL);
