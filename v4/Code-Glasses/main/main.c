/**
 * Smart Glasses LED Controller v4.2
 * 
 * Changes from v4.1:
 *   - Added BLE OTA firmware update support
 *   - OTA commands: 0xA8 (start), 0xA9 (finish), 0xAA (cancel)
 *   - New characteristics: 0xFF02 (OTA data), 0xFF03 (status notify)
 *   - LED task pauses during OTA, sleep blocked during OTA
 *   - Version bump to v4.2
 * 
 * Changes from v4.0:
 *   - Strobe and breathing are now INDEPENDENT modes
 *   - 0xA3 (breathing) = smooth breathing only, no strobe
 *   - 0xA1 (strobe) = strobe at given Hz with breathing envelope
 *   - 0xA5 (override) = static hold, stops ALL animation (strobe + breathing)
 *   - Boot: starts session with strobe ON (same as v4.0)
 * 
 * Features:
 *   - BLE OTA firmware updates (no wired programmer needed after first flash)
 *   - Timed sessions with auto-sleep at end
 *   - Linear progression of breathing pattern (hold times grow)
 *   - Optional strobe (0xA1 enables, 0xA3 disables)
 *   - PWM1 only (GPIO27), PWM2 commented out (hardware tied together)
 *   - BLE commands for configuring parameters
 *   - BLE static override (0xA5) to hold fixed duty, stops all animation
 *   - Hall sensor sleep/wake (close arms = sleep, open = wake)
 *   - Power optimized: 80MHz CPU, 1kHz PWM, -12dBm BLE TX
 * 
 * BLE Service: 0x00FF
 *   Characteristic 0xFF01 (Control) - Read/Write:
 *     Single byte (0x00-0xFF)                      - Legacy: direct duty (0=clear, 255=dark)
 *     0xA1 [start_hz] [end_hz]                     - Enable strobe, set Hz range (1-50)
 *     0xA2 [brightness]                            - Set brightness 0-100%
 *     0xA3 [inh] [hold_in_end] [exh] [hold_out_end] - Set breathing (disables strobe)
 *     0xA4 [duration_minutes]                      - Set session duration (1-60 min)
 *     0xA5 [duty]                                  - Static override, stops ALL animation
 *     0xA6 [0x00]                                  - Resume / restart session
...
#define GATTS_NUM_HANDLES       12      /* service + 3 chars with descriptors */
...
static uint8_t start_hz = 12;
static uint8_t end_hz = 8;
static uint8_t strobe_enabled = 0;
static uint8_t inhale_time = 40;
static uint8_t exhale_time = 40;
static uint8_t hold_in_end = 40;
static uint8_t hold_out_end = 40;
                                        ...
                                        esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_ADD_CHAR_EVT: {
        ...
        if (char_add_step == 0) {
            ...
            /* Add Status characteristic (0xFF03) */
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (handle == ctrl_char_handle && len > 0) {
            ...
            if (len == 1) {
                ...
                uint8_t cmd = value[0];
                switch (cmd) {
                case 0xA1:  /* Strobe: [0xA1] [start_hz] [end_hz] */
                    if (len >= 3) {
                        start_hz = value[1];
                        end_hz = value[2];
                        ...
                        session_active = 1;
                        session_ended = 0;
                        ESP_LOGI(TAG, "Strobe ON: %d->%d Hz", start_hz, end_hz);
                    }
                    break;
                ...
                case 0xA3:  /* Breathing: [0xA3] [inh] [hold_in] [exh] [hold_out] */
                    if (len >= 5) {
                        inhale_time = value[1];
                        hold_in_end = value[2];
                        exhale_time = value[3];
                        hold_out_end = value[4];
                        strobe_enabled = 0;
                        override_active = 0;
                        session_start_tick = xTaskGetTickCount();
                        session_active = 1;
                        session_ended = 0;
                        ESP_LOGI(TAG, "Breathing: %.1f/%.1f/%.1f/%.1f",
...
static void led_task(void *param)
{
    ...
    while (1) {
        ...
        if (now - last_log >= (30000 / portTICK_PERIOD_MS)) {
            ...
            ESP_LOGI(TAG, "%.0f%% | Strobe:%s | %.1fHz | %lus left",
                     progress * 100, strobe_enabled ? "ON" : "OFF",
                     current_hz, (unsigned long)remaining_s);
        }

        /* Breathing hold times */
        ...
        switch (breath_phase) {
            case 0: breath_brightness = bp * 100.0f; break;
            case 1: breath_brightness = 100.0f; break;
            case 2: breath_brightness = (1.0f - bp) * 100.0f; break;
            case 3: breath_brightness = 0.0f; break;
        }
        ...
        if (strobe_enabled) {
            ...
            uint32_t strobe_period_ms = (uint32_t)(1000.0f / current_hz);
            uint32_t strobe_on_ms = strobe_period_ms * 3 / 4;
            uint32_t strobe_off_ms = strobe_period_ms / 4;
            if (strobe_on_ms < 1) strobe_on_ms = 1;
            if (strobe_off_ms < 1) strobe_off_ms = 1;
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "===========================================");
    ESP_LOGI(TAG, "Smart Glasses v4.2 (OTA enabled)");
