 ...
 * - esp_timer strobe: 1µs resolution, default 10Hz, BLE-adjustable 1-40Hz
 ...
 * 
 * CHANGELOG v4.9:
 * - Fixed OTA finish crash: drain pending esp_ota_write() calls before
 *   esp_ota_end() to prevent deadlock between BTC task and OTA task
 * - esp_timer strobe from v4.8, AB freq command, deferred OTA from v4.6
 ******************************************************************************/
...
#define DEFAULT_STROBE_HZ       10      /* 10Hz default strobe */
...
static volatile uint8_t strobe_hz = DEFAULT_STROBE_HZ;
...
static void strobe_start(void) {
    ...
    uint64_t half_period_us = 500000ULL / strobe_hz;
    ...
    ESP_LOGI(TAG, "Strobe started: %dHz (%llu µs half-period)", 
             strobe_hz, half_period_us);
}
...
static void led_task(void *param) {
    ...
    while (1) {
        ...
        if (xTaskGetTickCount() - last_log_tick >= pdMS_TO_TICKS(30000)) {
            ...
            if (strobe_enabled) {
                ESP_LOGI(TAG, "Strobe %dHz @ %d%%, %lu sec remaining", 
                         strobe_hz, brightness, remaining_sec);
            } else {
...
static void process_command(uint8_t *data, uint16_t len) {
    ...
    switch (cmd) {
        ...
        case 0xA6:  /* Resume strobe */
            ...
            ESP_LOGI(TAG, "Strobe resumed: %dHz", strobe_hz);
            break;
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  Strobe: %dHz (esp_timer) @ %d%% brightness", DEFAULT_STROBE_HZ, DEFAULT_BRIGHTNESS);
