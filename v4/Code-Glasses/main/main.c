 ...
 * 
 * CHANGELOG v4.9.8:
 * - gptimer hardware timer at 100µs ISR replaces esp_timer (zero RTOS jitter)
 * - DDS phase accumulator for exact average strobe frequency (±100µs max error)
 * - Direct LEDC register writes in IRAM (ISR-safe, bypasses flash-resident driver)
 * - All else unchanged from v4.9.7
 ******************************************************************************/
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.9.8"
static const char *TAG = "SG_v4.9.8";
...
#define TICKS_PER_SEC           10000   /* 100µs tick rate */
 ...
 * 
 * AC: toggle every AC_HALF_TICKS (50 × 100µs = 5ms = 100Hz)
 * Strobe: DDS phase accumulator. Phase wraps at TICKS_PER_SEC (10000).
...
static bool IRAM_ATTR drive_timer_cb(gptimer_handle_t timer,
    ...
    /* ── Strobe DDS phase accumulator ── */
    if (led_mode == LED_MODE_STROBE && session_active && !in_ota_mode) {
        strobe_acc += strobe_dhz;
        if (strobe_acc >= TICKS_PER_SEC) strobe_acc -= TICKS_PER_SEC;

        uint32_t dark_threshold = (uint32_t)TICKS_PER_SEC * strobe_duty_pct / 100;
        effective_duty = (strobe_acc < dark_threshold) ? brightness : 0;
    }
...
static void drive_timer_init(void) {
    ...
    ESP_LOGI(TAG, "Drive timer started (100µs gptimer ISR / 100Hz AC)");
}
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  AC Drive: 100Hz gptimer ISR (GPIO26/27)");
