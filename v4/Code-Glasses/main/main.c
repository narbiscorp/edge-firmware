/*******************************************************************************
 * Smart Glasses Firmware v4.9
 * 
 * Based on v4.7. Replaced vTaskDelay-based strobe with esp_timer for
 * microsecond-precision strobe frequency control (0.1–40Hz).
 * 
 * FEATURES:
 * - AC differential drive: GPIO26/GPIO27 alternate every 10ms (50Hz AC)
 * - esp_timer strobe: 1µs resolution, default 0.5Hz, BLE-adjustable 1-40Hz
 * - OTA firmware updates via BLE (deferred to dedicated task)
 ...
 * - AA 00               Cancel OTA
 * - AB [hz]             Set strobe frequency 1-40 Hz
 * 
 ...
 * 
 * STROBE ARCHITECTURE (v4.8+):
 * Previous versions used vTaskDelay counting in the led_task to time strobe
 * transitions, which quantized frequency to multiples of the 10ms FreeRTOS
 * tick. v4.9 uses esp_timer (hardware timer, 1µs resolution from 40MHz XTAL)
 * to fire a callback at the exact strobe half-period. The callback toggles
 * effective_duty between brightness and 0. The led_task now only manages
 * session timeout. Strobe frequency is fully decoupled from both the
 * FreeRTOS tick rate and the AC drive timing.
 * 
 * CHANGELOG v4.9.1:
 * - Default strobe: 0.5Hz
 * - Internal frequency now stored as deci-Hz for sub-Hz precision
 * - AB command still accepts 1-40 Hz (integer)
 ******************************************************************************/
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.9.1"
static const char *TAG = "SG_v4.9.1";
 ...
 * Strobe: driven by esp_timer at 1µs resolution, fully independent of tick rate
 ******************************************************************************/
...
#define DEFAULT_STROBE_DHZ      5       /* 0.5Hz default strobe (deci-Hz) */
#define MIN_STROBE_HZ           1
#define MAX_STROBE_HZ           40
...
static volatile uint8_t strobe_phase = 0;    /* 0 = dark (tinted), 1 = clear */
static volatile uint16_t strobe_dhz = DEFAULT_STROBE_DHZ;  /* Deci-Hz (5 = 0.5Hz, 100 = 10Hz, 400 = 40Hz) */
...
/*******************************************************************************
 * STROBE TIMER (esp_timer — 1µs resolution from 40MHz XTAL)
 * 
 * Fires at the strobe half-period. Each callback toggles effective_duty
 * between brightness and 0. Fully decoupled from FreeRTOS tick rate and
 * AC drive timing. CPU cost: ~2-5µs per callback, negligible even at 40Hz.
 ******************************************************************************/
...
static void strobe_start(void) {
    ...
    uint64_t half_period_us = 5000000ULL / strobe_dhz;
    esp_timer_stop(strobe_timer);  /* safe even if not running */
    strobe_phase = 0;
    effective_duty = brightness;
    esp_timer_start_periodic(strobe_timer, half_period_us);
    ESP_LOGI(TAG, "Strobe started: %d.%dHz (%llu us half-period)", 
             strobe_dhz / 10, strobe_dhz % 10, half_period_us);
}
...
static void set_strobe_freq(uint8_t hz) {
    if (hz < MIN_STROBE_HZ) hz = MIN_STROBE_HZ;
    if (hz > MAX_STROBE_HZ) hz = MAX_STROBE_HZ;
    strobe_dhz = hz * 10;
    if (strobe_enabled && session_active) {
        strobe_start();  /* restart at new frequency */
    }
}
 ...
 * LED CONTROL TASK
 * 
 * Manages session lifetime. Strobe timing is handled by esp_timer.
 * Starts the strobe timer on session begin, stops it on session end / OTA.
 ******************************************************************************/
static void led_task(void *param) {
    uint32_t last_log_tick = 0;
    
    ESP_LOGI(TAG, "LED task started - session manager");
    
    /* Start session and strobe */
    session_active = true;
    ...
    while (1) {
        ...
        if (xTaskGetTickCount() - last_log_tick >= pdMS_TO_TICKS(30000)) {
            ...
            if (strobe_enabled) {
                ESP_LOGI(TAG, "Strobe %d.%dHz @ %d%%, %lu sec remaining", 
                         strobe_dhz / 10, strobe_dhz % 10, brightness, remaining_sec);
            } else {
                ESP_LOGI(TAG, "Static %d%%, %lu sec remaining", 
...
static void ota_do_finish(void) {
    ...
    /* Wait for all pending esp_ota_write() calls to complete.
     * Data chunks are written in the BTC task (GATT callback) while this
     * runs in the OTA task. The OTA API is not thread-safe for the same
     * handle — calling esp_ota_end() while esp_ota_write() is still
     * executing will deadlock or corrupt the flash. Watch ota_bytes_written
     * until it stabilizes (no new bytes for 100ms). */
    {
        ...
        ESP_LOGI(TAG, "OTA writes drained: %lu bytes total", ota_bytes_written);
    }
...
static void process_command(uint8_t *data, uint16_t len) {
    ...
    switch (cmd) {
        ...
        case 0xA6:  /* Resume strobe */
            ...
            ESP_LOGI(TAG, "Strobe resumed: %d.%dHz", strobe_dhz / 10, strobe_dhz % 10);
            break;
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "  Strobe: %d.%dHz (esp_timer) @ %d%% brightness", 
             DEFAULT_STROBE_DHZ / 10, DEFAULT_STROBE_DHZ % 10, DEFAULT_BRIGHTNESS);
    ...
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "Commands: A2=brightness A4=session A5=override A6=resume A7=sleep AB=freq");
