/**
 * Smart Glasses LED Controller v4.1
 * 
 * Changes from v4.0:
...
// Advertising parameters - match original for compatibility
static esp_ble_adv_params_t adv_params = {
    .adv_int_min        = 0x20,   // 20ms (original)
    .adv_int_max        = 0x40,   // 40ms (original)
...
static uint8_t strobe_enabled = 0;      // 0 = breathing only, 1 = strobe active
            ...
            switch (cmd) {
                case 0xA1:  // Set strobe range: [0xA1] [start_hz] [end_hz]
                    if (param->write.len >= 3) {
                        ...
                        strobe_enabled = 1;  // Enable strobe mode
                        override_active = 0;
                        session_start_tick = xTaskGetTickCount();
                        session_active = 1;
                        session_ended = 0;
                        ESP_LOGI(TAG, "Strobe ENABLED: %d->%d Hz", start_hz, end_hz);
                    }
                    break;
                ...
                case 0xA3:  // Set breathing: [0xA3] [inh] [hold_in_end] [exh] [hold_out_end]
                    if (param->write.len >= 5) {
                        ...
                        strobe_enabled = 0;  // Disable strobe, breathing only
                        override_active = 0;
                        session_start_tick = xTaskGetTickCount();
                        session_active = 1;
                        session_ended = 0;
                        ESP_LOGI(TAG, "Breathing (no strobe): %.1f/0->%.1f/%.1f/0->%.1f",
                ...
                case 0xA5:  // Static override: [0xA5] [duty 0-100]
                    if (param->write.len >= 2) {
                        ...
                        override_active = 1;
                        session_active = 0;
                        strobe_enabled = 0;  // Clear strobe on override
                        pwm1_setduty(override_duty);
...
static void led_task(void *param)
{
    ...
    while (1) {
        ...
        if (now - last_log >= (30000 / portTICK_PERIOD_MS)) {
            ...
            ESP_LOGI(TAG, "Progress: %.0f%% | Strobe: %s | Hz: %.1f | Breath: %.1f/%.1f/%.1f/%.1f | Remaining: %lus",
                     progress * 100, strobe_enabled ? "ON" : "OFF", current_hz,
        ...
        if (strobe_enabled) {
            // STROBE MODE: Apply strobe (75% duty: dark 3/4, clear 1/4)
            ...
            uint32_t strobe_on_ms = strobe_period_ms * 3 / 4;   // dark time (75%)
            uint32_t strobe_off_ms = strobe_period_ms / 4;      // clear time (25%)
            ...
            // BREATHING-ONLY MODE: Smooth PWM update at ~50Hz (20ms)
            pwm1_setduty(duty);
...
void app_main(void)
{
    ...
    // Start session immediately on boot with strobe enabled
    session_start_tick = xTaskGetTickCount();
    session_active = 1;
    session_ended = 0;
    strobe_enabled = 1;  // Boot with strobe ON

    // Create LED task
    xTaskCreate(led_task, "led_task", 4096, NULL, 1, NULL);

    ESP_LOGI(TAG, "============================================");
    ESP_LOGI(TAG, "Smart Glasses v4.1");
    ESP_LOGI(TAG, "BLE Name: %s", DEVICE_NAME);
    ESP_LOGI(TAG, "Session: %d min | Strobe: %d->%d Hz | Strobe: ON",
             session_minutes, start_hz, end_hz);
