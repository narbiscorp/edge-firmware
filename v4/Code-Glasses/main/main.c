/*******************************************************************************
 * Smart Glasses Firmware v4.13.6
 *
 ...
 * integration → LF resonance peak with ±1 bin window → coherence
 * score matching client's Lehrer/Vaschillo formula. Emitted on
 * 0xFF03 status char as 0xF2 packet once per second.
 *
 ...
 ******************************************************************************/
#define FIRMWARE_VERSION "4.13.6"
static const char *TAG = "SG_v4.13.6";
...
static void coh_compute(void) {
    ...
    /* Band integration — Task Force 1996 bands for display/diagnostics.
     * Bin indices (0.015625 Hz/bin):
     *   VLF: 0.003-0.04 → bins 0-2  (clamp low end to skip DC)
    ...
    float total_bands = vlf + lf + hf;  /* Display metric ("Task Force TP") */

    /* v4.13.5 FIX: coherence denominator must be the FULL-SPECTRUM total,
     * not just VLF+LF+HF. Client (dashboard v13.10) sums tp over all
     * frequency bins:
     *
     *   for(let i=0;i<nF;i++){ tp += psd[i]*df; ... }
     *
     * Bins above 0.4 Hz (frequencies 0.41-2.0 Hz in our 4Hz grid) carry
     * significant energy from ectopic beats, detection jitter, and
     * interpolation artifacts — especially on noisy sessions. Previous
     * firmware versions used VLF+LF+HF only as the denominator, which
     * artificially inflated coherence on real-world (noisy) data
     * because the denominator was systematically 2-4× smaller than
     * client's. Now matches. */
    float total = 0.0f;
    for (int i = 1; i < COH_GRID_N / 2; i++) total += psd[i];  /* skip DC */

    /* LF resonance peak (Lehrer/Vaschillo method, matches client v13.10):
     * 1. Find argmax within LF band (0.04-0.15 Hz = bins 3-9)
     * 2. Sum PSD in a narrow WINDOW around peak (±0.015 Hz = ±1 bin at our
     *    0.0156 Hz resolution) — captures the peak's shoulders, not just
     *    its single-bin maximum. A sharp true resonance has most of its
     *    power concentrated in 2-3 bins, and ignoring the shoulders
     *    systematically under-reports coherence.
     * 3. Coherence = (window_power / total_power) × 250, clamp 0-100
     *
     * v4.13.4 FIX: previous versions (v4.13.0–v4.13.3) used single peak
     * bin AND squared the ratio. Both were wrong. Client formula is
     * ratio × 250, window-summed. Firmware now matches. */
    int peak_bin = 3;
    ...
    /* Sum power in ±1 bin window around peak (inclusive of peak itself) */
    float peak_window_pow = psd[peak_bin];
    if (peak_bin > 0) peak_window_pow += psd[peak_bin - 1];
    if (peak_bin < COH_GRID_N/2 - 1) peak_window_pow += psd[peak_bin + 1];

    float coherence = 0.0f;
    if (total > 1e-6f) {
        float ratio = peak_window_pow / total;
     ...
     * The "total_power" field in the 0xF2 packet represents Task Force TP
     * (VLF+LF+HF) for consistency with client display — that's what the
     * dashboard labels "FW Total Power". The full-spectrum `total` used
     * for the coherence denominator is internal only. */
    float scale = 1.0f;
    float maxband = total_bands;
    ...
    coh_state.total_power    = (uint16_t)(total_bands * scale);
...
static void coherence_task(void *arg) {
    ESP_LOGI(TAG, "Coherence task v4.13.6 started (Fs=%.1fHz, N=%d, window=%.0fs)",
...
static void ppg_task(void *arg) {
    ...
    ESP_LOGI(TAG, "PPG task v4.13.6 started @ %d Hz (GPIO%d)",
